...
<!-- The input parameters from the command line -->
 ...
 <xsl:template match="XREF_LINK/DB[contains(.,'NA-STUDY')]">
  ...
  <xsl:result-document href="{concat('output/', $output-dir, '/', $acc-number, '/', 'i_', $acc-number, '.txt')}" method="text">
...
<xsl:value-of select="isa:single-name-value('Comment[SRA broker]', $broker-name)"/>
 ...
 <xsl:template match="XREF_LINK/DB[contains(.,'NA-SAMPLE')]">
  <xsl:result-document href="{concat('output/', $output-dir, '/', $acc-number, '/', 's_', $acc-number, '.txt')}" method="text">
 ...
 <xsl:template match="experiments/experiment" mode="distinct-exp">
  <xsl:result-document href="{concat('output/', $output-dir, '/', $acc-number, '/', 'a_', lower-case(@library-strategy), '-', lower-case(@library-source), '.txt')}" method="text">
