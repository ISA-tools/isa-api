...
from isatools.model import Characteristic, OntologyAnnotation, OntologySource, StudyFactor
...
ELEMENT_TYPES = dict(
    SCREEN=SCREEN, RUN_IN=RUN_IN, WASHOUT=WASHOUT, FOLLOW_UP=FOLLOW_UP, OBSERVATION_PERIOD=OBSERVATION_PERIOD
)
...
DURATION_FACTOR_ = dict(
    name="DURATION",
    type=OntologyAnnotation(term="time"),
    display_singular="DURATION VALUE",
    display_plural="DURATION VALUES",
    values=set(),
)
...
BASE_FACTORS_ = (
    dict(
        name="AGENT",
        type=OntologyAnnotation(term="perturbation agent"),
        display_singular="AGENT VALUE",
        display_plural="AGENT VALUES",
        values=set(),
    ),
    dict(
        name="INTENSITY",
        type=OntologyAnnotation(term="intensity"),
        display_singular="INTENSITY VALUE",
        display_plural="INTENSITY VALUES",
        values=set(),
    ),
    DURATION_FACTOR_,
)
...
SEQUENCE_ORDER_FACTOR_ = dict(name="Sequence Order", type=OntologyAnnotation(term="sequence order"))

SEQUENCE_ORDER_FACTOR = StudyFactor(name=SEQUENCE_ORDER_FACTOR_["name"], factor_type=SEQUENCE_ORDER_FACTOR_["type"])
