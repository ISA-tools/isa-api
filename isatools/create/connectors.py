...
from isatools.create.model import NonTreatment, SampleAndAssayPlan, StudyArm, StudyCell, StudyDesign, Treatment
from isatools.model import Characteristic, FactorValue, OntologyAnnotation, OntologySource
...
def _map_ontology_annotation(annotation, expand_strings=False):
    ...
    if isinstance(annotation, dict):
        res = OntologyAnnotation(term=annotation["term"])
        ...
        if isinstance(nodes, dict):
            ...
            for candidate_param_name, param_values in nodes.items():
                ...
                if candidate_param_name[0] == "#" and not isinstance(param_values, list):
                    ...
                    prepared_nodes[param_name] = [_map_ontology_annotation(param_value) for param_value in param_values]
...
def assay_ordered_dict_to_template(assay_ord_dict):
    ...
    """
    res = dict()
    res["measurement_type"] = _reverse_map_ontology_annotation(
        assay_ord_dict.get("measurement_type", None), compress_strings=True
    )
    res["technology_type"] = _reverse_map_ontology_annotation(
        assay_ord_dict.get("technology_type", None), compress_strings=True
    )
    res["workflow"] = []
    for name, nodes in assay_ord_dict.items():
        ...
        if isinstance(nodes, dict):
            ...
            serialized_nodes = {}
        res["workflow"].append([_reverse_map_ontology_annotation(name), serialized_nodes])
    return res
...
def _generate_element(datascriptor_element_dict):
    ...
    if "agent" in datascriptor_element_dict:
        ...
        element = Treatment(
            element_type=_map_ontology_annotation(intervention_type)
...
def _generate_sample_dict_from_config(datascriptor_sample_type_config, arm_name, epoch_no):
    return dict(
        ...
        characteristics_category=_map_ontology_annotation(
            datascriptor_sample_type_config.get("characteristicCategory", ORGANISM_PART), expand_strings=True
        ),
...
def _generate_characteristics_from_observational_factor(observational_factor_dict):
    ...
    value = (
        _map_ontology_annotation(observational_factor_dict["value"], expand_strings=True)
        if observational_factor_dict["isQuantitative"] is False
        else observational_factor_dict["value"]
    )
    unit = (
        _map_ontology_annotation(observational_factor_dict["unit"], expand_strings=True)
        if observational_factor_dict["isQuantitative"] is True
        else None
    )
...
def generate_assay_ord_dict_from_config(datascriptor_assay_config, arm_name, epoch_no):
    ...
    res["technology_type"] = _map_ontology_annotation(datascriptor_assay_config["technology_type"], expand_strings=True)
    res["selected_sample_types"] = list(
        map(_map_ontology_annotation, datascriptor_assay_config["selectedSampleTypes"][arm_name][epoch_no])
    )
        ...
        if "#replicates" in node:
            ...
            for candidate_param_name, param in node.items():
                ...
                if candidate_param_name[0] == "#":
                    ...
                    if not param["values"]:
                        raise ValueError(
                            "Missing values for Protocol Param {}".format(
                                candidate_param_name["term"]
                                if isinstance(candidate_param_name, dict)
            ...
            if "characteristics_value" in node:
                if not node["characteristics_value"]["values"]:
                    raise ValueError(
                        "Missing values for Characteristic {}".format(
                            node["characteristics_category"]["term"]
                            if isinstance(node["characteristics_category"], dict)
                ...
                prepared_nodes = [
                    dict(
                        ...
                        characteristics_category=_map_ontology_annotation(
                            node["characteristics_category"], expand_strings=True
                        ),
...
def generate_study_design(datascriptor_study_config):
    ...
    for arm_ix, arm_dict in enumerate(study_design_config["arms"]["selected"]):
        ...
        for epoch_ix, epoch_dict in enumerate(arm_dict["epochs"]):
            ...
            elements = [
                _generate_element(element_dict)
                for element_dict in filter(lambda el: el["id"] in element_ids, study_design_config["elements"])
            ]
            cell_name = "A{}E{}".format(arm_ix, epoch_ix)
            cell = StudyCell(name=cell_name, elements=elements)
            sample_type_dicts = [
                _generate_sample_dict_from_config(ds_sample_config, arm_dict["name"], epoch_ix)
                for ds_sample_config in study_design_config["samplePlan"]
                if ds_sample_config["selectedCells"][arm_dict["name"]][epoch_ix]
                and ds_sample_config["sampleTypeSizes"][arm_dict["name"]][epoch_ix]
            ]
            assay_ord_dicts = [
                generate_assay_ord_dict_from_config(ds_assay_config, arm_dict["name"], epoch_ix)
        ...
        arm = StudyArm(
            ...
            source_characteristics=[
                _generate_characteristics_from_observational_factor(obs_factor_dict)
