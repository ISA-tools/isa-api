...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:import href="../sra/isa-functions-biocrates.xsl"/>
   
    <xsl:output method="text" encoding="UTF-8"/>
    <xsl:strip-space elements="*"/>

    <xsl:param name="MYPATHVAR" />
...
<xsl:template match="data">
    <!-- a test to assess that the output directory by passing the value as param via the command line invocation of the saxon is OK -->
    <!--<xsl:text>The value of MYPATHVAR is: </xsl:text>-->
    <!--<xsl:value-of select="concat('toto: ',$MYPATHVAR)"/>-->
    <!-- END OF COMMENT-->

    <xsl:text>&#9;</xsl:text>
    <xsl:variable name="investigationfile" select="concat($MYPATHVAR,data,'i_inv_biocrates.txt')[normalize-space()]"></xsl:variable>
...
<xsl:text>
    ...
    <xsl:template name="generate-investigation-section">
        ...
        <xsl:text>
INVESTIGATION
Investigation Identifier&#9;11111111
Investigation Title
...
<xsl:template match="metabolite" name="metabolite_desc_posi">
    ...
    <xsl:variable name="datafile" select="concat($MYPATHVAR,data,'maf-positive.txt')[normalize-space()]"></xsl:variable>
...
<xsl:template match="metabolite" name="metabolite_desc_nega">
    ...
    <xsl:variable name="usedOP" select="@usedOP"/>
        ...
        <xsl:variable name="datafile" select="concat($MYPATHVAR,$usedOP,'-maf-negative.txt')[normalize-space()]"></xsl:variable>
        <xsl:value-of select="$datafile[normalize-space()]" /> 
        <xsl:result-document href="{$datafile}">
            <xsl:copy-of select=".[normalize-space()]"/>
...
<!--     template to create ISA s_study table               -->
    ...
    <xsl:variable name="studyfile" select="concat($MYPATHVAR,data,'s_study_biocrates.txt')[normalize-space()]"/>
...
<xsl:template name="assay-negativeMode" match="plate"  priority="1">
    ...
    <xsl:variable name="assaynegativefile" select="concat($MYPATHVAR,data,'a_biocrates_assay_negative_mode.txt')[normalize-space()]"></xsl:variable>
...
<xsl:template match="plate" name="assay-positiveMode" priority="1">
    ...
    <xsl:variable name="assaypositivefile" select="concat($MYPATHVAR,data,'a_biocrates_assay_positive_mode.txt')[normalize-space()]"></xsl:variable>
