...
def update_current_file(input_file):
    """
    :param input_file: a MetaboLightsAssayMaster.tsv resource
    :type input_file: str
    :return: latest_version -> string , path to latest_version
    """
    ...
    try:
        ...
        if os.path.exists(os.path.join('resources/mtbls', new_folder_name)):  # reusing if already there
            ...
            os.makedirs(os.path.join('resources/mtbls', new_folder_name))   # creating the resource
            if ".tsv" in input_file:
                shutil.move(MTBLS_ASSAY_DEF_FILE, os.path.join(dst, "MetaboLightsAssayMaster.tsv.previous"))
                shutil.move(input_file, os.path.join('resources/mtbls/current', "MetaboLightsAssayMaster.tsv"))
                latest_version = os.path.join('resources/mtbls/current', "MetaboLightsAssayMaster.tsv")
...
def check_file_for_updates(old_file, resource_url):
    """
    :param old_file: name of the currently stored file
    :type old_file: str
    :param resource_url: url to the EMBL-EBI Metabolights resource
    :type resource_url: str
    :return: mtbls_master_file a resource file corresponding to the latest version of the Metabolights definitions
    """
...
def sha256_checksum(filename, block_size=65536):
    """
    :param filename: the name of the file to checksum
    :type  filename: str
    :param block_size: integer to set to the size of checksum blocks
    :type block_size: int
    :return: a sha256 checksum for the input file
    """
...
def load_terms_from_mtbls_owl(ontology_file):
    """
    a method to load all subclasses of Instruments from Metabolights.owl ontology file.
    :param ontology_file: a owl file from EMBL Metabolights Metabolights.owl
    :type ontology_file: str
    :return: a list of class labels, a list of subclasses from the root class, the ontology file checksum
    """
    ...
    try:
        ...
        vocab_graph.parse(ontology_file, format='xml')
...
def build_params_slim(protocol_row_record, assay_dictionary, param_prefix, tech, mtbls_class_names,
    ...
    :param protocol_row_record: a row corresponding to a protocol workflow definition
    as found MTBLSAssayMaster.tsv
    :type protocol_row_record:
    :param assay_dictionary: an `isa-create-mode` assay dictionary stub
    :type assay_dictionary: dict
    :param param_prefix:
    :type param_prefix: str
    :param tech:
    :type tech: str
    :param mtbls_class_names:
    :type mtbls_class_names: list
    :param mtbls_associated_subclasses:
    :type mtbls_associated_subclasses: dict
    :return: assay_dictionary
    """
    # getting all subclasses of Instruments from Metabolights.owl

    def make_param_values(current_protocol_params, current_associated_subclasses):
        """
        :param current_protocol_params:
        :type current_protocol_params: list
        :param current_associated_subclasses:
        :type current_associated_subclasses: dict
        :return: param_setup a dictionary with parameters per protocol type
        """
        for protocol_param in current_protocol_params:
            ...
            for this_element in current_associated_subclasses:
...
def parse_mtbls_assay_def(file, class_names, associated_subclasses, owl_sha256):
    """
            ...
            for row in assay_master:
                if "-protocol" in row[0]:
                    ...
                    if "NMR" in tech_type:
                        ...
                        assay_dict["mtbls_owl_sha256"] = str(owl_sha256)
                        ...
                        assay_dict = build_params_slim(protocol_row, assay_dict, assay_param_prefix,
                                                       tech_type, class_names, associated_subclasses)
                        ...
                        assay_dict = build_params_slim(protocol_row, assay_dict, param_prefix, tech_type,
                                                       class_names, associated_subclasses)
                        all_assays.append(assay_dict)
                        counter += 1
...
def create_isa_datascriptor_assay_def():
    ...
    owl_file = check_file_for_updates(MTBLS_CV_OWL, "https://raw.githubusercontent.com/EBI-Metabolights/MtblsWS-Py/master/tests/Metabolights.owl")
    mtbls_class_names, mtbls_associated_subclasses, mtbls_owl_sha256 = load_terms_from_mtbls_owl(owl_file)
