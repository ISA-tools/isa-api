...
def _build_metadata_df(ISA):
    metadata_df = pd.DataFrame(columns=("MAGE-TAB Version", "Investigation Title", "Public Release Date", "SDRF File"))
    sdrf_filenames = _get_sdrf_filenames(ISA)
    metadata_df.loc[0] = ["1.1", ISA.title, ISA.public_release_date, sdrf_filenames[0]]
    for i, sdrf_filename in enumerate(sdrf_filenames):
        if i == 0:
            pass
        else:
            metadata_df.loc[i] = ["", "", "", sdrf_filename]
    return metadata_df


def _build_exp_designs_df(ISA):
    exp_designs_df = pd.DataFrame(
        columns=(
            "Experimental Design",
...
def write_idf_file(inv_obj, output_path):
    ...
    with open(
        os.path.join(
            output_path,
            "{}.idf.txt".format(
                investigation.identifier if investigation.identifier != "" else investigation.filename[2:-3]
            ),
        ),
        "wb",
    ) as idf_fp:
...
class MageTabParser(object):
    ...
    def parse_idf(self, in_filename):
        ...
        self.parse_ontology_sources(
            self._idfdict.get("termsourcename", []),
        ...
        self.parse_protocols(
            self._idfdict.get("protocolname", []),
        ...
        self.parse_comments(
            {key: self._idfdict[key] for key in [x for x in self._idfdict.keys() if x.startswith("comment[")]}
        )
        ...
        for factor, factortype, tsr, tan in zip_longest(factors, factortypes, tsrs, tans, fillvalue=""):
            if factor != "":  # only add if there's a factor name
                factortype_oa = OntologyAnnotation(
                    term=factortype, term_source=self._ts_dict.get(tsr), term_accession=tan
                )
    ...
    def parse_publications(self, pubmedids, dois, authorlists, titles, statuses, statustans, statustsrs):
        for pubmedid, doi, authorlist, title, status, statustsr, statustan in zip_longest(
            pubmedids, dois, authorlists, titles, statuses, statustans, statustsrs, fillvalue=""
        ):
            ...
            if pubmedid != "" or doi != "" or title != "":
                ...
                publication = Publication(
                    pubmed_id=pubmedid, doi=doi, author_list=authorlist, title=title, status=statusoa
                )
    ...
    def parse_protocols(self, names, ptypes, tsrs, tans, descriptions, parameterslists, hardwares, softwares, contacts):
        for name, ptype, tsr, tan, description, parameterslist, hardware, software, contact in zip_longest(
            names, ptypes, tsrs, tans, descriptions, parameterslists, hardwares, softwares, contacts, fillvalue=""
        ):
            if name != "":  # only add if there's a name
                ...
                protocol = Protocol(
                    name=name,
                    protocol_type=protocoltype_oa,
                    description=description,
                    parameters=list(
                        map(
                            lambda x: ProtocolParameter(parameter_name=OntologyAnnotation(term=x)),
                            parameterslist.split(";") if parameterslist is not None else "",
                        )
                    ),
                )
                protocol.comments = [
                    Comment(name="Protocol Hardware", value=hardware),
    ...
    def infer_missing_metadata(self):
        ...
        if defaultassay is None and (
            "transcriptionprof" in get_squashed(S.title) or "geneexpressionprof" in get_squashed(S.title)
        ):
            defaultassay = Assay(
        ...
        self.ISA.filename = "i_{0}investigation.txt".format(
            self.ISA.identifier + "_" if self.ISA.identifier != "" else self.ISA.identifier
        )
    ...
    def _get_measurement_and_tech(design_type):
        ...
        if re.match("(?i).*ChIP-Chip.*", design_type):
            assay = Assay(
                measurement_type=OntologyAnnotation(term="protein-DNA binding site identification"),
                technology_type=OntologyAnnotation(term="DNA microarray"),
                technology_platform="ChIP-Chip",
            )
        if (
            re.match("(?i).*RNA-seq.*", design_type)
            or re.match("(?i).*RNA-Seq.*", design_type)
            or re.match("(?i).*transcription profiling by high throughput sequencing.*", design_type)
        ):
            assay = Assay(
                measurement_type=OntologyAnnotation(term="transcription profiling"),
        ...
        if re.match("(?i).*methylation profiling by array.*", design_type):
            assay = Assay(
                measurement_type=OntologyAnnotation(term="DNA methylation profiling"),
                technology_type=OntologyAnnotation(term="DNA microarray"),
                technology_platform="Me-Chip",
            )
        if re.match("(?i).*comparative genomic hybridization by array.*", design_type):
            assay = Assay(
                measurement_type=OntologyAnnotation(term="comparative genomic hybridization"),
                technology_type=OntologyAnnotation(term="DNA microarray"),
                technology_platform="CGH-Chip",
            )
        if re.match(".*genotyping by array.*", design_type):
            assay = Assay(
                measurement_type=OntologyAnnotation(term="SNP analysis"),
                technology_type=OntologyAnnotation(term="DNA microarray"),
        ...
        with open(in_filename, encoding="utf-8") as in_fp:
            ...
            for i, col in enumerate(df.columns):
                if col.lower().startswith("term source ref") and df.columns[i - 1].lower().startswith("protocol ref"):
                    ...
                    # after protocol ref
                elif col.lower().startswith("term source ref") and df.columns[i - 1].lower().startswith(
                    "array design ref"
                ):
                    pass
    ...
    def split_assay(self, fp):
        ...
        for line in fp.readlines():
            ...
            if (
                is_hybridization_assay
                and ("genomicdna" in sqline or "genomic_dna" in sqline)
            ...
            if (
                (is_hybridization_assay and not contains_antibody_in_header)
                and "rna" in sqline
            ...
            if (
                not is_hybridization_assay and ("genomicdna" in sqline or "genomic_dna" in sqline)
