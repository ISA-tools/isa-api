...
class IsatabToJsonWriterV2():
    ...
    def createInvestigationNode(self, json_structures, rec):
        for meta in rec.metadata:
            json_structures[self.commonFunctions.makeAttributeName(meta, 'Investigation')] = rec.metadata[meta]
    ...
    def createListOfAttributesArray(self, properties, toBeRemovedTag):
        json_list_struct = []
        for onto in properties:
            json_item_struct = {}
            for item in onto:
                json_item_struct[self.commonFunctions.makeAttributeName(item, toBeRemovedTag)] = onto[item]
    ...
    def createListOfAttributes(self, json_structures, properties, tagName):
        ...
        for onto in properties:
            json_item_struct = {}
            for item in onto:
                json_item_struct[self.commonFunctions.makeAttributeName(item, 'Term Source')] = onto[item]
            json_list_struct.append(json_item_struct)
    ...
    def createStudyNode(self, json_structures, rec, work_dir, json_dir, isSingleStructure):
        json_inner_struct = {}
        for meta in rec.metadata:
            json_inner_struct[self.commonFunctions.makeAttributeName(meta, 'Study')] = rec.metadata[meta]
        json_inner_struct["designDescriptors"] = self.createListOfAttributesArray(rec.design_descriptors, 'Study')
        json_inner_struct["publications"] = self.createListOfAttributesArray(rec.publications, 'Study Publication')
        json_inner_struct["factors"] = self.createListOfAttributesArray(rec.factors, 'Study')
        ...
        for sp in rec.protocols:
            json_sp = {}
            for i_sp in sp:
                json_sp[self.commonFunctions.makeAttributeName(i_sp, 'Study Protocol')] = sp[i_sp].decode('ascii', errors='ignore')
            json_study_protocol.append(json_sp)
        json_inner_struct["protocols"] = json_study_protocol
        json_inner_struct["people"] = self.createListOfAttributesArray(rec.contacts, 'Study Person')
        ...
        for assay in rec.assays:
            ...
            for i_assay in assay:
                json_assay_structure[self.commonFunctions.makeAttributeName(i_assay, 'Study Assay')] = assay[i_assay]
