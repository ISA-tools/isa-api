...
def parse(isatab_ref):
    ...
    if os.path.isdir(isatab_ref):
        fnames = glob.glob(os.path.join(isatab_ref, "i_*.txt")) + glob.glob(os.path.join(isatab_ref, "*.idf.txt"))
        assert len(fnames) == 1
        isatab_ref = fnames[0]
...
class StudyAssayParser:
    ...
    def __init__(self, base_file):
        ...
        self._col_quals = ("Performer", "Date", "Unit", "Term Accession Number", "Term Source REF")
        self._col_types = {
            ...
            "node_assay": ("Assay Name", "Data Transformation Name", "Normalization Name"),
    ...
    def parse(self, rec):
        ...
        for study in rec.studies:
            source_data = self._parse_study(study.metadata["Study File Name"], self._col_types["node"])
            ...
            if source_data:
                ...
                for assay in study.assays:
                    ...
                    assay_data = self._parse_study(assay["Study Assay File Name"], self._col_types["node"])
            ...
            for line in reader:
                ...
                for processing_index in processing_indices:
                    ...
                    parameters_indices = find_in_between(
                        all_parameters_indices, processing_index, next_processing_index
                    )
                    ...
                    input_node_indices = [
                        self._build_node_index(input_headers[i], input_values[i]) for i, x in enumerate(input_values)
                    ]

                    output_values = [line[hgroups[x][0]] for i, x in enumerate(output_indices)]
                    output_node_indices = [
                        self._build_node_index(output_headers[i], output_values[i]) for i, x in enumerate(output_values)
                    ]
                    ...
                    if assay_name:
                        ...
                        try:
                            unique_process_name = input_process_map[
                                qualifier_indices_string + input_node_indices_string
                            ]
                            if not (unique_process_name.startswith(processing_name)):
                                raise KeyError
                        except KeyError:
                            try:
                                unique_process_name = output_process_map[
    ...
    def _line_keyvals(self, line, header, hgroups, htypes, out):
        ...
        out = self._line_by_type(line, header, hgroups, htypes, out, "attribute", self._collapse_attributes)
        out = self._line_by_type(line, header, hgroups, htypes, out, "processing", self._collapse_attributes)
        out = self._line_by_type(line, header, hgroups, htypes, out, "parameter", self._collapse_attributes)
        return out

    @staticmethod
    def _line_by_type(line, header, hgroups, htypes, out, want_type, collapse_quals_fn=None):
...
_node_str = """       * Node -> {name} {type} {index}
         attributes: {attributes}
         derivesFrom: {derivesFrom}
         metadata: {md}"""

_process_node_str = """       * Process Node ->  {name} {type}
