...
from isatools.isatab.utils import find_gt, find_lt, get_squashed
...
class IsaTabParser(object):
    ...
    def parse_investigation(self, in_filename):
        ...
        self.parse_ontology_sources_section(
            ...
            {k: isecdict[k] for k in isecdict.keys() if k.startswith("ontologysourcereference.")},
        )
        ...
        self.parse_publications_section(
            ...
            {k: isecdict[k] for k in isecdict.keys() if k.startswith("investigationpublications.")},
        )
        ...
        for ssecdict in ssecdicts:
            ...
            self.parse_publications_section(
                ...
                {k: ssecdict[k] for k in ssecdict.keys() if k.startswith("studypublications.")},
            )
            self.parse_people_section(
                self.ISA.studies[-1],
    ...
    def parse_ontology_sources_section(self, names, files, versions, descriptions, comments_dict):
        ...
        for name, file, version, description in zip_longest(names, files, versions, descriptions):
            i += 1
            current_onto_source = OntologySource(name=name, file=file, version=version, description=description)
            for k, v in comments_dict.items():
                if i < len(v) > 0:
                    current_onto_source.comments.append(Comment(name=next(iter(_RX_COMMENT.findall(k))), value=v[i]))
    ...
    def parse_investigation_section(
        self, identifiers, titles, descriptions, submissiondates, publicreleasedates, comments_dict
    ):
        for identifier, title, description, submissiondate, publicreleasedate in zip_longest(
            identifiers, titles, descriptions, submissiondates, publicreleasedates
        ):
            ...
            for k, v in comments_dict.items():
                if len(v) > 0:
                    self.ISA.comments.append(
                        Comment(name=next(iter(_RX_COMMENT.findall(k))), value=";".join(v) if len(v) > 1 else v[0])
                    )
    ...
    def parse_study_section(self, identifiers, titles, descriptions, submissiondates, publicreleasedates, filenames):
        for identifier, title, description, submissiondate, publicreleasedate, filename in zip_longest(
            identifiers, titles, descriptions, submissiondates, publicreleasedates, filenames
        ):
            study = Study(
    ...
    def parse_publications_section(
        self, obj, pubmedids, dois, authorlists, titles, statuses, statustans, statustsrs, comments_dict
    ):
        i = 0
        for pubmedid, doi, authorlist, title, status, statustsr, statustan in zip_longest(
            pubmedids, dois, authorlists, titles, statuses, statustans, statustsrs
        ):
            i += 1
            statusoa = OntologyAnnotation(
                term=status, term_source=self._ts_dict.get(statustsr), term_accession=statustan
            )
            publication = Publication(pubmed_id=pubmedid, doi=doi, author_list=authorlist, title=title, status=statusoa)
            for k, v in comments_dict.items():
                if i < len(v) > 0:
                    publication.comments.append(Comment(name=next(iter(_RX_COMMENT.findall(k))), value=v[i]))
    ...
    def parse_people_section(
        ...
        for i, contact in enumerate(self.ISA.studies[-1].contacts):
            for k, v in comments_dict.items():
                if len(v) > 0:
                    contact.comments.append(Comment(name=next(iter(_RX_COMMENT.findall(k))), value=v[i]))
...
def isatab_get_data_files_list_command(input_path, output, json_query=None, galaxy_parameters_file=None):
    ...
    :return: None
    """
    log.info("Getting data files for study %s. Writing to %s.", input_path, output.name)
    if json_query:
...
def isatab_get_data_files_collection_command(input_path, output_path, json_query=None, galaxy_parameters_file=None):
    ...
    :return: None
    """
    log.info("Getting data files for study %s. Writing to %s.", input_path, output_path)
    if json_query:
        log.debug("This is the specified query:\n%s", json_query)
    else:
        log.debug("No query was specified")
            ...
            if factor_selection is None:
                ...
                for factor_name, factor_value in factor_selection.items():
                    if "Factor Value[{}]".format(factor_name) in list(df.columns.values):
                        matches = df.loc[df["Factor Value[{factor}]".format(factor=factor_name)] == factor_value][
                            "Sample Name"
                        ].items()
                        ...
                        for indx, match in matches:
                            ...
                            if len([r for r in results if r["sample"] == sample_name]) == 1:
                                ...
                                results.append(
                                    {"sample": sample_name, "data_files": [], "query_used": factor_selection}
                                )
...
def isatab_get_factor_names_command(input_path, output):
    ...
    :return: None
    """
    log.info("Getting factors for study %s. Writing to %s.", input_path, output.name)
...
def isatab_get_factor_values_command(input_path, factor, output):
    ...
    """
    log.info(
        "Getting values for factor {factor} in study {input_path}. Writing to {output_file}.".format(
            factor=factor, input_path=input_path, output_file=output.name
        )
    )
    fvs = set()
    ...
    for table_file in glob.iglob(os.path.join(input_path, "[a|s]_*")):
        with open(os.path.join(input_path, table_file)) as fp:
            ...
            if "Factor Value[{factor}]".format(factor=factor_name) in list(df.columns.values):
                for _, match in df["Factor Value[{factor}]".format(factor=factor_name)].iteritems():
                    try:
                        match = match.item()
...
def query_isatab(source_dir, output, galaxy_parameters_file=None):
    ...
    if mt and tt:
        for study in investigation.studies:
            matching_assays.extend(
                [x for x in study.assays if x.measurement_type.term == mt and x.technology_type.term == tt]
            )
    elif mt and not tt:
        for study in investigation.studies:
            matching_assays.extend([x for x in study.assays if x.measurement_type.term == mt])
    elif not mt and tt:
        for study in investigation.studies:
            matching_assays.extend([x for x in study.assays if x.technology_type.term == tt])
    else:
        for study in investigation.studies:
    ...
    factor_selection = {
        x.get("factor_name").strip(): x.get("factor_value").strip() for x in query.get("factor_selection", [])
    }
    ...
    if factor_selection:
        ...
        for f, v in factor_selection.items():
            for sample in assay_samples:
                for fv in [x for x in sample.factor_values if x.factor_name.name == f]:
    ...
    characteristics_selection = {
        x.get("characteristic_name").strip(): x.get("characteristic_value").strip()
        for x in query.get("characteristics_selection", [])
    }
        ...
        for c, v in characteristics_selection.items():
            if first_pass:
                for sample in final_fv_samples:
                    for cv in [x for x in sample.characteristics if x.category.term == c]:
                    ...
                    for source in sample.derives_from:
                        for cv in [x for x in source.characteristics if x.category.term == c]:
                            if isinstance(cv.value, OntologyAnnotation):
                                if cv.value.term == v:
                                    cv_samples.add(sample)
                            elif cv.value == v:
    ...
    parameters_selection = {
        x.get("parameter_name").strip(): x.get("parameter_value").strip() for x in query.get("parameter_selection", [])
    }
    ...
    for sample in final_samples:
        results.append({"sample_name": sample.name, "data_files": []})
    for result in results:
            ...
            with open(table_file) as fp:
                ...
                if parameters_selection:
                    for p, v in parameters_selection.items():
                        sample_pv_rows = sample_rows.loc[sample_rows["Parameter Value[{}]".format(p)] == v]
                        for node_label in data_node_labels:
                            if node_label in table_headers:
                                data_files.extend(list(sample_pv_rows[node_label]))
                    result["data_files"].extend(list(set(i for i in list(data_files) if str(i) not in ("nan", ""))))
                else:
                    for node_label in data_node_labels:
                        if node_label in table_headers:
                            data_files.extend(list(sample_rows[node_label]))
                    result["data_files"].extend(list(set(i for i in list(data_files) if str(i) not in ("nan", ""))))
    results_json = {"query": query, "results": results}
...
def isatab_get_factors_summary_command(input_path, output):
    ...
    :return: None
    """
    log.info("Getting summary for study %s. Writing to %s.", input_path, output.name)
    ISA = load(input_path)
    ...
    for table_file in iglob(path.join(input_path, "[a|s]_*")):
        ...
        for query in queries:
            ...
            if "Sample_Name" in df.columns:
                log.debug(
                    "Group: {query} / Sample_Name: {sample_name}".format(
                        query=query, sample_name=list(df2["Sample_Name"])
                    )
                )

            if "Source_Name" in df.columns:
                log.debug("Group: {} / Sources_Name: {}".format(query, list(df2["Source_Name"])))

            if "Raw_Spectral_Data_File" in df.columns:
                log.debug(
                    "Group: {query} / Raw_Spectral_Data_File: {filename}".format(
                        query=query[13:-2], filename=list(df2["Raw_Spectral_Data_File"])
                    )
                )
    return queries
