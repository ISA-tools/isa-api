...
from isatools.isatab.defaults import _RX_INDEXED_COL, log
...
def check_utf8(fp):
    ...
    with utf8_text_file_open(fp.name) as fp:
        ...
        if charset["encoding"] != "UTF-8" and charset["encoding"] != "ascii":
            ...
            log.warning(
                "File should be UTF-8 encoding but found it is '{0}' encoding with {1} confidence".format(
                    charset["encoding"], charset["confidence"]
                )
            )
            raise SystemError()
...
def check_term_source_refs_in_investigation(i_df):
    ...
    def check_study_term_sources_in_secton_field(section_label, pos, column_label):
        ...
        if len(diff) > 0:
            ...
            log.warning(
                "(W) In {} one or more of {} has not been declared in {}.{} section".format(
    ...
    if len(diff) > 0:
        ...
        log.warning(
            "(W) Investigation Publication Status Term Source REF {} "
    ...
    i_person_roles_term_source_ref = [
        i for i in i_df["i_contacts"]["Investigation Person Roles Term Source REF"].tolist() if i != ""
    ]
    ...
    if len(diff) > 0:
        ...
        log.warning(
            "(W) Investigation Person Roles Term Source REF {} has "
                            ...
                            if row not in ontology_sources_list:
                                if isinstance(row, float):
                                    if not math.isnan(row):
                                        ...
                                        validator_warnings.append(
                                            {"message": "Missing Term Source", "supplemental": sup_msg, "code": 3009}
                                        )
                                        ...
                                        log.warning(
                                            ...
                                            "sources {}".format(
                                                row + 1, object_index[x], y + 1, study_filename, list(oslist)
                                            )
                                        )
                                else:
                                    oslist = ontology_sources_list
...
def check_study_table_against_config(s_df, protocols_declared, config):
    ...
    for x, object in enumerate(object_index):
        if fields[x] != object[1]:
            validator_warnings.append(
                {
                    ...
                    "supplemental": "Unexpected heading found. Expected {} but found {} at column number {}".format(
            ...
            log.warning(
                "(W) Unexpected heading found. Expected {} but found {} at column number {}".format(
                    fields[x], object[1], object[0]
                )
            )
...
def check_assay_table_against_config(s_df, config):
    ...
    object_index = [
        (x, i)
        for x, i in enumerate(norm_columns)
        if i
        in [
            "Source Name",
            "Sample Name",
        ...
        if required_field not in columns:
            validator_warnings.append(
                {
                    ...
                    "supplemental": "In {} the required column {} missing from column headings".format(
                        filename, required_field
                    ),
                    "code": 4010,
                }
            )
            log.warning(
                "(W) In {} the required column {} missing from column headings".format(filename, required_field)
            )
        else:
            ...
            for y, cell in enumerate(df[required_field]):
                if not cell_has_value(cell):
                    ...
                    log.warning(
                        "(W) Cell at row {} in column '{}' has no "
