...
def load_investigation(fp):
    ...
    for i in range(0, len(df_dict["studies"])):
        ...
        check_labels("STUDY DESIGN DESCRIPTORS", labels_expected, df_dict["s_design_descriptors"][i])
        ...
        check_labels("STUDY PUBLICATIONS", labels_expected, df_dict["s_publications"][i])
        ...
        check_labels("STUDY CONTACTS", labels_expected, df_dict["s_contacts"][i])

    return df_dict


def validate(
    fp: TextIO, config_dir: str = default_config_dir, origin: str or None = None, rules: dict = None, log_level=None
) -> dict:
    """
    ...
    try:
        ...
        for i, study_df in enumerate(i_df_dict["studies"]):
            ...
            study_validator = ISAStudyValidator(
                validator=investigation_validator,
                study_index=i,
