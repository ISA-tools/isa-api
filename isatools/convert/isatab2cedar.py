            ...
            if isa_tab.metadata != {}:
                investigationObject = dict(
                    [
                        ...
                        (
                            "publicReleaseDate",
                            dict([("_value", isa_tab.metadata["Investigation Public Release Date"])]),
                        ),
        ...
        for study in studies:
            ...
            json_item = dict(
                [
                    ...
                    ("process", self.createProcessList(study.process_nodes, source_dict, sample_dict)),
                ]
            )
        ...
        for process_node_name in process_nodes:
            ...
            json_item = dict(
                [
                    ...
                    ("parameterValue", self.createParameterValueList(process_node)),
                ]
            )
            json_list.append(json_item)
        return json_list
            ...
            if nodes[node_index].ntype == "Sample Name":
                json_item = dict(
                    [
                        ...
                        ("characteristic", self.createCharacteristicList(nodes[node_index])),
                    ]
                )
                json_dict.update({node_index: json_item})
        return json_dict
        ...
        for assay in assays:
            json_item = dict(
                [
                    ...
                    (
                        "measurementType",
                        dict([("_value", assay.metadata["Study Assay Measurement Type Term Accession Number"])]),
                    ),
