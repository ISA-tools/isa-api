...
def nihdcc2isa_convert(json_path, output_path):
    ...
    inv_person = Person(
        ...
        affiliation=(", ".join([project_json["department"], project_json["institution"]])),
        roles=[OntologyAnnotation(term="", term_source=obi, term_accession="https://purl.org/obo/OBI_1")],
    )
    ...
    if len(study_set_json) > 0:
        ...
        study = Study(
            ...
            design_descriptors=[
                OntologyAnnotation(
                    term=study_json["type"], term_source=obi, term_accession="https://purl.org/obo/OBI_1"
                )
            ],
        ...
        study_person = Person(
            ...
            roles=[
                OntologyAnnotation(
                    term="principal investigator", term_source=obi, term_accession="https://purl.org/obo/OBI_1"
                )
            ],
        )
        ...
        for i, protocol_json in enumerate(dcc_json["protocol"].values()):
            ...
            oa_protocol_type = OntologyAnnotation(
                term=oat_p, term_source=obi, term_accession="https://purl.org/obo/OBI_1"
            )
            study.protocols.append(
                Protocol(
                    name=protocol_json["id"],
                ...
                study.assays.append(
                    Assay(
                        ...
                        technology_type=OntologyAnnotation(
                            term="mass spectrometry", term_source=obi, term_accession="https://purl.org/obo/OBI_1"
                        ),
            ...
            if "NMR" in protocol_json["type"]:
                study.assays.append(
                    Assay(
                        measurement_type=OntologyAnnotation(
                            term="isotopomer analysis", term_source=obi, term_accession="https://purl.org/obo/OBI_111"
                        ),
                        technology_type=OntologyAnnotation(
                            term="nmr spectroscopy", term_source=obi, term_accession="https://purl.org/obo/OBI_1"
                        ),
        ...
        for subject_json in dcc_json["subject"].values():
            if "organism" in subject_json["type"]:
                ...
                sample = Sample(name=subject_json["id"], derives_from=subject_json["parentID"])
                characteristic_organismpart = Characteristic(
                    category=OntologyAnnotation(term="organism_part"),
                    value=OntologyAnnotation(
                        term=subject_json["tissue_type"], term_source=obi, term_accession="https://purl.org/obo/OBI_1"
                    ),
                )
                ...
                sample_collection_process = Process(executes_protocol=study.get_prot(subject_json["protocol.id"]))
        ...
        for sample_json in dcc_json["sample"].values():
            if "cells" in sample_json["type"]:
                ...
                if len([x for x in study.samples if x.name == sample_json["parentID"]]) == 0:
                    ...
                    material_separation_process.inputs.append(
                        [x for x in study.samples if x.name == sample_json["parentID"]][0]
                    )
                ...
                material_type = Characteristic(
                    ...
                    value=OntologyAnnotation(
                        term=sample_json["type"], term_source=obi, term_accession="https://purl.org/obo/OBI_xxxxxxx"
                    ),
                )
            ...
            if "protein_extract" in sample_json["type"]:
                ...
                material_type = Characteristic(
                    category=OntologyAnnotation(term="material_type"),
                    value=OntologyAnnotation(
                        term=sample_json["type"], term_source=obi, term_accession="https://purl.org/obo/OBI_1"
                    ),
                )
                material_out.characteristics.append(material_type)
            ...
            if "polar" in sample_json["type"]:
                ...
                material_type = Characteristic(
                    category=OntologyAnnotation(term="material_type", term_source=obi),
                ...
                datafile = DataFile(
                    filename="{filename}.txt".format(
                        filename="_".join(["mass_isotopomer-data", studyid, sample_json["id"]])
                    ),
                    label="Raw Data File",
                )
    ...
    for element in dcc_json["measurement"]:
        record = "\t".join(
            (
                dcc_json["measurement"][element]["compound"],
    ...
    try:
        with open(
            "{output_path}/{study_id}-maf-data-nih-dcc-json.txt".format(output_path=output_path, study_id=studyid), "w"
        ) as fh:
