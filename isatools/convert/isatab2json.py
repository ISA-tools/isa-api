...
SCHEMAS_PATH = join(
    os.path.dirname(os.path.realpath(__file__)), "..", "resources", "schemas", "isa_model_version_1_0_schemas", "core"
)
...
def convert(
    ...
    validate_first=True,
    config_dir=isatab.default_config_dir,
    ...
    if validate_first:
        ...
        with open(i_files[0], "r", encoding="utf-8") as validate_fp:
            report = isatab.validate(fp=validate_fp, config_dir=config_dir, log_level=logging.ERROR)
...
class ISATab2ISAjson_v1:
    ...
    def createComment(self, name, value):
        comment_json = dict([("name", name), ("value", value)])
        return comment_json
    ...
    def createProtocols(self, protocols, assays):
        ...
        protocol_json = dict(
            [
                ...
                ("name", "unknown"),
                ("protocolType", dict([("annotationValue", "")])),
                ("description", ""),
                ("uri", ""),
                ("version", ""),
                ("parameters", []),
    ...
    def createProtocolParameterList(self, protocol):
        ...
        for parameter_json in parameters_json:
            ...
            json_item = dict([("@id", parameter_identifier), ("parameterName", parameter_json)])
    ...
    def createOntologyAnnotation(self, name, termSource, termAccession):
        onto_ann = dict([("annotationValue", name), ("termSource", termSource), ("termAccession", termAccession)])
        return onto_ann
    ...
    def createOntologyAnnotationsFromStringList(self, object, inv_or_study, type):
        ...
        for i in range(0, len(name_array)):
            if not name_array[i]:
                continue
            onto_ann = self.createOntologyAnnotation(name_array[i], term_source_array[i], term_accession_array[i])
        ...
        for study in studies:
            ...
            studyJson = dict(
                [
                    ...
                    (
                        "processSequence",
                        self.createProcessSequence(
                            study.process_nodes, source_dict, sample_dict, material_dict, data_dict
                        ),
                    ),
                    ("assays", assay_list),
            ...
            if not unknown_used:
                try:
                    unknown_prot_index = protocol_list.index(
                        dict(
                            [
                                ("@id", "#protocol/unknown"),
                                ("name", "unknown"),
    ...
    def createProtocolComponentList(self, protocol):
        ...
        for component_type_json in components_type_json:
            ...
            json_item = dict([("componentName", component_name), ("componentType", component_type_json)])
            json_list.append(json_item)
            index += 1
        return json_list
    ...
    def createProcessSequence(self, process_nodes, source_dict, sample_dict, material_dict, data_dict):
        ...
        for process_node_name in process_nodes:
            ...
            previous_process_identifier = (
                self.getIdentifier("process", process_node.previous_process.name)
                if process_node.previous_process
                else ""
            )
            next_process_identifier = (
                self.getIdentifier("process", process_node.next_process.name) if process_node.next_process else ""
            )
            if process_node.assay_name:
                json_item = dict(
                    [
                        ...
                        (
                            "parameterValues",
                            self.createValueList(self.PARAMETER_VALUE, process_node_name, process_node),
                        ),
                        (
                            "inputs",
                            self.createInputList(
                                process_node.inputs, source_dict, sample_dict, material_dict, data_dict
                            ),
                        ),
    ...
    def createExecuteStudyProtocol(self, process_node_name, process_node):
        json_item = dict([("@id", self.getIdentifier("protocol", process_node.protocol))])
        return json_item
        ...
        for assay in assays:
            ...
            json_item = dict(
                [
                    ...
                    ("characteristicCategories", characteristics_categories_list),
                    ("unitCategories", unit_categories_list),
                    ("materials", dict([("samples", sample_list), ("otherMaterials", list(material_dict.values()))])),
                    ("dataFiles", list(data_dict.values())),
                    (
                        "processSequence",
                        self.createProcessSequence(
                            assay.process_nodes, source_dict, sample_dict, material_dict, data_dict
                        ),
                    ),
                ]
            )
    ...
    def createFromNodeComments(self, node):
        ...
        for key in [key for key in node.metadata.keys() if _RX_COMMENTS.match(key)]:
            comments.append(
                self.createComment(
                    _RX_COMMENTS.findall(key)[0],
                    getattr(node.metadata[key][0], _RX_COMMENTS.findall(key)[0].replace(" ", "_")),
                )
            )
        return comments
    ...
    def createMaterialDictionary(self, nodes):
        ...
        for node_index in nodes:
            if (
                nodes[node_index].ntype != "Source Name"
                and nodes[node_index].ntype != "Sample Name"
                and nodes[node_index].ntype.find("File") == -1
            ):
    ...
    def createCharacteristicsCategories(self, nodes):
        ...
        for node_index in nodes:
            ...
            for header in node.metadata:
                if (
                    (not header.startswith(self.CHARACTERISTICS))
                    and (not header == self.MATERIAL_TYPE)
                    and (not header == self.LABEL)
                ):
                    continue
    ...
    def createUnitsCategories(self, nodes):
        ...
        for node_index in nodes:
            ...
            for header in node.metadata:
                if (
                    not header.startswith(self.CHARACTERISTICS)
                    and not header.startswith(self.FACTOR_VALUE)
                    and not header.startswith(self.PARAMETER_VALUE)
                ):
                    continue
                ...
                json_item.update(
                    self.createOntologyAnnotation(
                        value_attributes.Unit, value_attributes.Term_Source_REF, value_attributes.Term_Accession_Number
                    )
                )
                json_list.append(json_item)
        return json_list
    ...
    def createValueList(self, column_name, node_name, node):
        ...
        for header in node.metadata:
            if (
                header.startswith(column_name)
                or header == self.MATERIAL_TYPE
                or header == self.LABEL
                or header == self.ARRAY_DESIGN_REF
            ):
                value_header = header.replace("]", "").split("[")[-1]
                ...
                if value_header is None or category_identifier is None:
                    try:
                        ...
                        value_json = dict([("value", value), ("unit", dict([("@id", unit_identifier)]))])
                            ...
                            value_json = dict(
                                [
                                    (
                                        ...
                                        self.createOntologyAnnotation(
                                            value,
                                            value_attributes.Term_Source_REF,
                    ...
                    try:
                        ...
                        try:
                            ...
                            value_json = dict([("category", dict([("@id", category_identifier)])), ("value", value)])
                            json_list.append(value_json)
        return json_list
