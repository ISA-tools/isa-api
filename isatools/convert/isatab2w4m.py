...
def read_args():
    ...
    parser.add_argument(
        ...
        + s1
        + ' Default is "'
        + dft_matrix_file.replace("%", "%%")
        + '".',
        dest="matrix_output",
        required=False,
        default=dft_matrix_file,
    )
    parser.add_argument(
        "-S",
...
def select_study(investigation_file, study_filename=None):
    ...
    if len(investigation.studies) > 1 and study_filename is None:
        error(
            'The investigation file "{}" contains more than one study. You need to select one of them.'.format(
                investigation_file
            )
        )
    ...
    if study_filename is not None:
        ...
        if study is None:
            error('Study "{0}" not found in investigation file "{1}".'.format(study_filename, investigation_file))
    ...
    for df in assay.data_files:
        ...
        if m is not None:
            if data_filename is not None:
                error(
                    'Found two data files ("{0}" and "{1}") in assay "{2}".'.format(
                        data_filename, df.filename, assay.filename
                    )
                )
...
def make_sample_metadata(study_df: object, assay_df, sample_names, normalize=True):
    ...
    if normalize:
        ...
        sample_metadata = sample_metadata.set_axis(
            axis=1, labels=make_names(sample_metadata.axes[1].tolist(), uniq=True)
        )
...
def make_variable_metadata(measures_df, sample_names, variable_names, normalize=True):
    ...
    # Normalize
    if normalize:
        variable_metadata = variable_metadata.set_axis(
            axis=1, labels=make_names(variable_metadata.axes[1].tolist(), uniq=True)
        )
...
def convert2w4m(input_dir, study_filename=None, assay_filename=None, all_assays=False):
    ...
    for assay in assays:
        ...
        sample_names = get_sample_names(assay_df=assay_df, measures_df=measures_df)
        sample_metadata = make_sample_metadata(
            study_df=study_df, assay_df=assay_df, sample_names=sample_names, normalize=True
        )
        variable_metadata = make_variable_metadata(
            measures_df=measures_df, sample_names=sample_names, variable_names=variable_names, normalize=True
        )
        sample_variable_matrix = make_matrix(
            measures_df=measures_df, sample_names=sample_names, variable_names=variable_names, normalize=True
        )
        w4m_assays.append(
            dict(
...
def filter_na_values(assays, samp_na_filtering=None, var_na_filtering=None):
    ...
    for assay in assays:
        if samp_na_filtering is not None:
            ...
            assay["samp"] = samp
            assay["mat"] = assay["mat"].drop(labels=removed_sample_names.tolist(), axis=1)
        ...
        if var_na_filtering is not None:
            ...
            assay["mat"] = assay["mat"].drop(
                labels=[assay["mat"].axes[0][i] for i in removed_variable_names_index], axis=0
            )
...
def convert(
    ...
    assay_filename=None,
    all_assays=None,
    samp_na_filtering=None,
    var_na_filtering=None,
):
    # Convert assays to W4M format
    assays = convert2w4m(
        input_dir=input_dir, study_filename=study_filename, assay_filename=assay_filename, all_assays=all_assays
    )
    ...
    write_assays(
        assays, output_dir=output_dir, samp_file=sample_output, var_file=variable_output, mat_file=matrix_output
    )
