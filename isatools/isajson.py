...
def load(fp):
    ...
    for ontologySourceReference_json in investigation_json[
        ...
        term_source_dict[
            ontology_source_reference.name] = ontology_source_reference
        investigation.ontology_source_references.append(
    ...
    for publication_json in investigation_json["publications"]:
        publication = Publication(
            ...
            status=OntologyAnnotation(
                ...
                term_source=term_source_dict[publication_json[
                    "status"]["termSource"]]
            )
        )
        try:
    ...
    for study_json in investigation_json["studies"]:
        for assay_json in study_json["assays"]:
            for assay_characteristics_category_json \
                ...
                characteristic_category = OntologyAnnotation(
                    ...
                    term=assay_characteristics_category_json[
                        "characteristicType"]["annotationValue"],
                    term_source=term_source_dict[
                        assay_characteristics_category_json[
                            "characteristicType"]["termSource"]],
                    term_accession=assay_characteristics_category_json[
                        "characteristicType"]["termAccession"],
                )
                # study.characteristic_categories.append(
                # characteristic_category)
                categories_dict[characteristic_category.id] = \
                    characteristic_category
    for study_json in investigation_json["studies"]:
        process_dict = dict()
        study = Study(
        ...
        try:
            ...
            pass
        for study_characteristics_category_json in study_json[
                "characteristicCategories"]:
            characteristic_category = OntologyAnnotation(
                id_=study_characteristics_category_json["@id"],
                term=study_characteristics_category_json[
                    "characteristicType"]["annotationValue"],
                term_source=term_source_dict[
                    study_characteristics_category_json[
                        "characteristicType"]["termSource"]],
                term_accession=study_characteristics_category_json[
                    "characteristicType"]["termAccession"],
            )
        ...
        for study_publication_json in study_json["publications"]:
            study_publication = Publication(
                ...
                status=OntologyAnnotation(
                    ...
                    term_source=term_source_dict[study_publication_json[
                        "status"]["termSource"]],
                    term_accession=study_publication_json[
                        "status"]["termAccession"],
                )
            )
            try:
                study_publication.comments = get_comments(
                    study_publication_json)
            except KeyError:
                pass
        ...
        for design_descriptor_json in study_json["studyDesignDescriptors"]:
            design_descriptor = OntologyAnnotation(
                ...
                term_source=term_source_dict[design_descriptor_json[
                    "termSource"]]
            )
        ...
        for protocol_json in study_json["protocols"]:
            protocol = Protocol(
                ...
                protocol_type=OntologyAnnotation(
                    ...
                    term_accession=protocol_json[
                        "protocolType"]["termAccession"]
                    if "termAccession" in protocol_json[
                        "protocolType"].keys() else "",
                    term_source=term_source_dict[protocol_json[
                        "protocolType"]["termSource"]]
                    if "termSource" in protocol_json[
                        "protocolType"].keys() else None,
                )
            )
            for parameter_json in protocol_json["parameters"]:
                parameter = ProtocolParameter(
                    ...
                    parameter_name=OntologyAnnotation(
                        ...
                        term_source=term_source_dict[parameter_json[
                            "parameterName"]["termSource"]],
                        term_accession=parameter_json[
                            "parameterName"]["termAccession"]
                    )
                )
            ...
            for component_json in protocol_json["components"]:
                component = ProtocolComponent(
                    ...
                    component_type=OntologyAnnotation(
                        ...
                        term_source=term_source_dict[component_json[
                            "componentType"]["termSource"]],
                        term_accession=component_json[
                            "componentType"]["termAccession"]
                    )
                )
        ...
        for factor_json in study_json["factors"]:
            factor = StudyFactor(
                ...
                factor_type=OntologyAnnotation(
                    ...
                    term_source=term_source_dict[factor_json[
                        "factorType"]["termSource"]]
                )
            )
                ...
                if isinstance(value, dict):
                    try:
                        ...
                        value = OntologyAnnotation(
                            term=term,
                            term_source=term_source_dict[characteristic_json[
                                "value"]["termSource"]],
                            term_accession=characteristic_json[
                                "value"]["termAccession"])
                    except KeyError:
        ...
        for sample_json in study_json["materials"]["samples"]:
            ...
            for factor_value_json in sample_json["factorValues"]:
                ...
                factor_value = FactorValue(
                    factor_name=factors_dict[factor_value_json[
                        "category"]["@id"]])
                if isinstance(value, dict):
                    try:
                        value = OntologyAnnotation(
            ...
            try:
                for source_id_ref_json in sample_json["derivesFrom"]:
                    sample.derives_from.append(
                        sources_dict[source_id_ref_json["@id"]])
            except KeyError:
                sample.derives_from = []
        for study_process_json in study_json["processSequence"]:
            process = Process(
                id_=study_process_json["@id"],
                executes_protocol=protocols_dict[
                    study_process_json["executesProtocol"]["@id"]],
            )
            try:
            ...
            for parameter_value_json in study_process_json["parameterValues"]:
                if isinstance(parameter_value_json["value"], int) \
                        or isinstance(parameter_value_json["value"], float):
                    ...
                    try:
                        parameter_value.value = OntologyAnnotation(
                            term=parameter_value_json[
                                "value"]["annotationValue"],
                            term_accession=parameter_value_json[
                                "value"]["termAccession"],
                            term_source=term_source_dict[
                                parameter_value_json["value"]["termSource"]],)
                    except TypeError:
            ...
            for input_json in study_process_json["inputs"]:
                ...
                if input_ is None:
                    raise IOError(
                        "Could not find input node in sources or "
                        "samples dicts: " + input_json["@id"])
            ...
            for output_json in study_process_json["outputs"]:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                if output is None:
                    raise IOError("Could not find output node in sources or "
                                  "samples dicts: " + output_json["@id"])
        ...
        for study_process_json in study_json["processSequence"]:  # 2nd pass
            try:
                ...
                process_dict[study_process_json[
                    "@id"]].prev_process = process_dict[prev_proc]
            except KeyError:
                pass
            ...
            try:
                ...
                process_dict[study_process_json[
                    "@id"]].next_process = process_dict[next_proc]
            except KeyError:
                pass
        ...
        for assay_json in study_json["assays"]:
            ...
            assay = Assay(
                measurement_type=OntologyAnnotation(
                    ...
                    term_accession=assay_json[
                        "measurementType"]["termAccession"],
                    term_source=term_source_dict[assay_json[
                        "measurementType"]["termSource"]]
                ),
                technology_type=OntologyAnnotation(
                    term=assay_json["technologyType"]["annotationValue"],
                    term_accession=assay_json[
                        "technologyType"]["termAccession"],
                    term_source=term_source_dict[
                        assay_json["technologyType"]["termSource"]]
                ),
            ...
            for assay_unit_json in assay_json["unitCategories"]:
                unit = OntologyAnnotation(id_=assay_unit_json["@id"],
                                          term=assay_unit_json[
                                              "annotationValue"],
                                          term_source=term_source_dict[
                                              assay_unit_json["termSource"]],
                                          term_accession=assay_unit_json[
                                              "termAccession"])
                units_dict[unit.id] = unit
            ...
            for data_json in assay_json["dataFiles"]:
                ...
                try:
                    data_file.derives_from = samples_dict[
                        data_json["derivesFrom"][0]["@id"]]
                except KeyError:
            ...
            for assay_characteristics_category_json in assay_json[
                    "characteristicCategories"]:
                characteristic_category = OntologyAnnotation(
                    id_=assay_characteristics_category_json["@id"],
                    term=assay_characteristics_category_json[
                ...
                study.characteristic_categories.append(characteristic_category)
                categories_dict[
                    characteristic_category.id] = characteristic_category
            other_materials_dict = dict()
            for other_material_json in assay_json[
                ...
                for characteristic_json in other_material_json[
                        "characteristics"]:
                    characteristic = Characteristic(
                        ...
                        value=OntologyAnnotation(
                            term=characteristic_json[
                                "value"]["annotationValue"],
                            term_source=term_source_dict[
                                characteristic_json["value"]["termSource"]],
                            term_accession=characteristic_json[
                                "value"]["termAccession"],
                        )
                    )
            ...
            for assay_process_json in assay_json["processSequence"]:
                process = Process(
                    ...
                    executes_protocol=protocols_dict[assay_process_json[
                        "executesProtocol"]["@id"]]
                )
                try:
                ...
                if process.executes_protocol.protocol_type.term \
                        == "data collection" and \
                        assay.technology_type.term == "DNA microarray":
                    process.name = assay_process_json["name"]
                elif process.executes_protocol.protocol_type.term \
                ...
                for input_json in assay_process_json["inputs"]:
                    ...
                    if input_ is None:
                        raise IOError(
                            "Could not find input node in samples or "
                            "materials or data dicts: " + input_json["@id"])
                ...
                for output_json in assay_process_json["outputs"]:
                    ...
                    if output is None:
                        raise IOError(
                            ...
                            "materials or data dicts: " + output_json["@id"])
                    process.outputs.append(output)
                for parameter_value_json in \
                        assay_process_json["parameterValues"]:
                    if "category" in parameter_value_json.keys():
                        if parameter_value_json["category"]["@id"] == \
                                "#parameter/Array_Design_REF":  # Special case
                            process.array_design_ref = parameter_value_json[
                                "value"]
                        elif isinstance(
                                parameter_value_json["value"], int) or \
                                isinstance(
                                    parameter_value_json["value"], float):
                            parameter_value = ParameterValue(
                                category=parameters_dict[
                                    parameter_value_json["category"]["@id"]],
                                value=parameter_value_json["value"],
                            ...
                            if "unit" in parameter_value_json.keys():
                                parameter_value.unit = units_dict[
                                    parameter_value_json["unit"]["@id"]]
                            process.parameter_values.append(parameter_value)
                        else:
                            parameter_value = ParameterValue(
                                category=parameters_dict[
                            ...
                            try:
                                ...
                                parameter_value.value = \
                                    parameter_value_json["value"]
                            process.parameter_values.append(parameter_value)
                    else:
                        log.warning(
                            "warning: parameter category not found for "
                            "instance {}".format(parameter_json))
                ...
                for assay_process_json in assay_json["processSequence"]:
                    ...
                    try:
                        ...
                        process_dict[assay_process_json[
                            "@id"]].next_process = process_dict[next_proc]
                    except KeyError:
                        pass
...
def get_io_ids_in_process_sequence(study_json):
    ...
    return [elem for iterabl in [
        [i["@id"] for i in process["inputs"]] + [
            o["@id"] for o in process[
                "outputs"]]
        for process in all_process_sequences] for elem in iterabl]
...
def check_material_ids_declared_used(study_json, id_collector_func):
    ...
    if not is_node_ids_used:
        warnings.append({
            ...
            "supplemental": "{} not used in any inputs/outputs in {}"
            .format(node_ids, io_ids_in_process_sequence),
            "code": 1017
        })
        log.warning("(W) Not all node IDs in {} used by inputs/outputs {}"
                    .format(node_ids, io_ids_in_process_sequence))
...
def check_material_ids_not_declared_used(study_json):
    ...
    if len(set(io_ids_in_process_sequence)) - len(set(node_ids)) > 0:
        ...
        errors.append({
            ...
            "supplemental": "Inputs/outputs in {}  not found in sources, "
                            "samples, materials or datafiles declarations"
                            .format(list(diff)),
            "code": 1005
        })
        log.error("(E) There are some inputs/outputs IDs {} not found in "
                  "sources, samples, materials or data files declared"
                  .format(list(diff)))
    ...
    for process in process_sequence_json:
        try:
            if process["previousProcess"]["@id"] not in process_ids:
                errors.append({
                    ...
                    "supplemental": "previousProcess {} in process {} does "
                                    "not refer to another process in sequence"
                                    .format(process["previousProcess"]["@id"],
                                            process["@id"]),
                    "code": 1006
                })
        ...
        try:
            if process["nextProcess"]["@id"] not in process_ids:
                errors.append({
                    "message": "Missing Process link",
                    "supplemental": "nextProcess {} in process {} does not "
                                    "refer to another process in "
                                    "sequence".format(process["nextProcess"][
                                        "@id"], process["@id"]),
                    "code": 1006
                })
...
def check_process_protocol_ids_usage(study_json):
    ...
    if len(set(protocol_ids_used) - set(protocol_ids_declared)) > 0:
        ...
        log.error("(E) There are protocol IDs {} used in a study or assay "
                  "process sequence not declared".format(list(diff)))
        ...
        warnings.append({
            ...
            "supplemental": "protocol IDs declared {} not used".format(
                list(diff)),
            "code": 1019
        })
        log.warning("(W) There are some protocol IDs declared {} not used in "
                    "any study or assay process sequence".format(list(diff)))
...
def get_parameter_value_parameter_ids(study_json):
    ...
    study_pv_parameter_ids = [elem for iterabl in
                              [[parameter_value["category"]["@id"]
                                for parameter_value in process[
                                    "parameterValues"]] for process in
                               study_json["processSequence"]]
                              for elem in iterabl]
    for assay in study_json["assays"]:
...
def check_protocol_parameter_ids_usage(study_json):
    ...
    if len(set(protocols_used) - set(protocols_declared)) > 0:
        ...
        warnings.append({
            "message": "Protocol parameter declared in a protocol but never "
                       "used",
            "supplemental": "protocol declared {} are not used".format(
                list(diff)),
            "code": 1020
        })
        log.warning("(W) There are some protocol parameters declared {} not "
                    "used in any study or assay process".format(list(diff)))
...
def get_characteristic_category_ids_in_assay_materials(assay_json):
    ...
    return [elem for iterabl in [[characteristic["category"]["@id"]
                                 ...
                                 if "characteristics" in
                                    material.keys() else [] for material in
                                 assay_json["materials"]["samples"] +
                                 assay_json["materials"]["otherMaterials"]]
            for elem in iterabl]
...
def check_characteristic_category_ids_usage(studies_json):
    ...
    for study_json in studies_json:
        characteristic_categories_declared += \
            get_characteristic_category_ids(study_json)
        for assay in study_json["assays"]:
            characteristic_categories_declared_in_assay = \
                get_characteristic_category_ids(assay)
            characteristic_categories_declared += \
                characteristic_categories_declared_in_assay
        characteristic_categories_used += \
            get_characteristic_category_ids_in_study_materials(study_json)
        for assay in study_json["assays"]:
            characteristic_categories_used_in_assay = \
                get_characteristic_category_ids_in_assay_materials(assay)
            characteristic_categories_used += \
                characteristic_categories_used_in_assay
    if len(set(characteristic_categories_used) -
        ...
        log.warning("(W) There are characteristic categories declared {} that "
                    "have not been used in any source or sample "
                    "characteristic".format(list(diff)))
...
def check_study_factor_usage(study_json):
    ...
    if len(set(factors_used) - set(factors_declared)) > 0:
        ...
        log.error("(E) There are study factors {} used in a sample factor "
                  "value that have not been not declared".format(list(diff)))
        ...
        log.warning("(W) There are some study factors declared {} that have "
                    "not been used in any sample factor value".format(
                        list(diff)))
...
def get_study_unit_category_ids_in_materials_and_processes(study_json):
    ...
    study_characteristics_units_used = [
        elem for iterabl in [[characteristic["unit"]["@id"]
                              ...
                              for characteristic in material[
                                  "characteristics"]] for material in
                             study_json["materials"]["sources"] +
                             study_json["materials"]["samples"]]
        for elem in iterabl]
    study_factor_value_units_used = [
        elem for iterabl in [[factor_value["unit"]["@id"]
                              if "unit" in factor_value.keys() else None
                              for factor_value in material[
                                  "factorValues"]] for material in
                             study_json[
                                 "materials"]["samples"]] for elem in iterabl]
    parameter_value_units_used = [
        elem for iterabl in[[parameter_value["unit"]["@id"]
        ...
        for elem in iterabl]
    return [x for x in study_characteristics_units_used +
            study_factor_value_units_used + parameter_value_units_used
            if x is not None]
...
def get_assay_unit_category_ids_in_materials_and_processes(assay_json):
    ...
    assay_characteristics_units_used = [
        elem for iterabl in [[characteristic["unit"]["@id"]
                             ...
                             else None for material in assay_json[
                                 "materials"]["otherMaterials"]]
        for elem in iterabl]
    parameter_value_units_used = [
        elem for iterabl in[[parameter_value["unit"]["@id"]
        ...
        elem in iterabl]
    return [x for x in assay_characteristics_units_used +
            parameter_value_units_used if x is not None]
...
def check_unit_category_ids_usage(study_json):
    ...
    units_used = get_study_unit_category_ids_in_materials_and_processes(
        study_json)
    log.info("Getting units used (assay)...")
    for assay in study_json["assays"]:
        units_used.extend(
            get_assay_unit_category_ids_in_materials_and_processes(assay))
    ...
    if len(set(units_used) - set(units_declared)) > 0:
        ...
        log.error("(E) There are units {} used in a material or parameter "
                  "value that have not been not declared".format(list(diff)))
        ...
        log.warning("(W) There are some units declared {} that have not been "
                    "used in any material or parameter value".format(
                        list(diff)))
    ...
    with open(fp.name, "rb") as fp:
        ...
        if charset["encoding"] is not "UTF-8" \
            ...
            warnings.append({
                ...
                "supplemental": "Encoding is '{0}' with confidence {1}"
                                .format(charset["encoding"],
                                        charset["confidence"]),
                "code": 10
            })
            log.warning("(W) File should be UTF-8 encoding but found it is "
                        "'{0}' encoding with {1} confidence"
    ...
    try:
        with open(investigation_schema_path) as fp:
            ...
            resolver = RefResolver(
                "file://" + investigation_schema_path, investigation_schema)
            validator = Draft4Validator(
                investigation_schema, resolver=resolver)
        ...
        log.fatal("(F) The JSON does not validate against the provided "
                  "ISA-JSON schemas!")
        log.fatal("Fatal error: " + str(ve))
        raise SystemError("(F) The JSON does not validate against the "
                          "provided ISA-JSON schemas!")
    ...
    def check_doi(doi_str):
        if doi_str is not "":
            if not _RX_DOI.match(doi_str):
                ...
                log.warning("(W) DOI {} does not conform to DOI format".format(
                    doi_str))
        ...
        for a_pos, assay in enumerate(study["assays"]):
            if assay["filename"] is "":
                warnings.append({
                    ...
                    "supplemental": "At study position {}, assay position {}"
                                    .format(s_pos, a_pos),
                    "code": 3005
                })
    ...
    def check_pubmed_id(pubmed_id_str):
        if pubmed_id_str is not "":
            if (_RX_PMID.match(pubmed_id_str) is None) and (
                    _RX_PMCID.match(pubmed_id_str) is None):
                warnings.append({
                    "message": "PubMed ID is not valid format",
                    "supplemental": "Found PubMedID {}".format(pubmed_id_str),
                    "code": 3003
                })
                log.warning("(W) PubMed ID {} is not valid format".format(
                    pubmed_id_str))
    for ipub in isa_json["publications"]:
    ...
    for study in isa_json["studies"]:
        for protocol in study["protocols"]:
            if protocol["name"] is "":
                ...
                log.warning("(W) A Protocol {} is missing Protocol Name, so "
                            "can't be referenced in ISA-tab".format(
                                protocol["@id"]))
            ...
            for parameter in protocol["parameters"]:
                if parameter["parameterName"] is "":
                    warnings.append({
                        ...
                        "supplemental": "Protocol Parameter @id={}".format(
                            parameter["@id"]),
                        "code": 1011
                    })
                    log.warning("(W) A Protocol Parameter {} is missing name, "
                                "so can't be referenced in ISA-tab".format(
                                    parameter["@id"]))
        ...
        for factor in study["factors"]:
            if factor["factorName"] is "":
                warnings.append({
                    ...
                    "supplemental": "Study Factor @id={}".format(
                        factor["@id"]),
                    "code": 1012
                })
                log.warning("(W) A Study Factor is missing name, so can't be "
                            "referenced in ISA-tab".format(factor["@id"]))
...
def check_ontology_sources(isa_json):
    ...
    for ontology_source in isa_json["ontologySourceReferences"]:
        if ontology_source["name"] is "":
            ...
            log.warning("(W) An Ontology Source Reference is missing "
                        "Term Source Name, so can't be referenced")
...
def check_term_source_refs(isa_json):
    ...
    walk_and_get_annotations(isa_json, collector)
    term_sources_used = [
        annotation["termSource"] for annotation in collector
        if annotation["termSource"] is not ""]
    if len(set(term_sources_used) - set(term_sources_declared)) > 0:
        ...
        log.warning("(W) There are some ontology sources declared {} that "
                    "have not been used in any annotation".format(list(diff)))
...
def check_term_accession_used_no_source_ref(isa_json):
    ...
    terms_using_accession_no_source_ref = [
        annotation for annotation in collector
        if annotation["termAccession"] is not "" and annotation[
            "termSource"] is ""]
    if len(terms_using_accession_no_source_ref) > 0:
        warnings.append({
            "message": "Missing Term Source REF in annotation",
            "supplemental": "Terms with accession but no source "
                            "reference {}".format(
                                terms_using_accession_no_source_ref),
            "code": 3010
        })
        log.warning("(W) There are ontology annotations with termAccession "
                    "set but no termSource referenced: {}"
        ...
        try:
            with open(file) as fp:
                ...
                if os.path.basename(file) == "protocol_definitions.json":
                    ...
                    configs[(config_dict["measurementType"],
                             config_dict["technologyType"])] = config_dict
            ...
            log.error("(E) Could not load configuration file {}".format(
...
def check_measurement_technology_types(assay_json, configs):
    try:
        ...
        errors.append({
            ...
            "supplemental": "Measurement {}/technology {}".format(
...
def check_study_and_assay_graphs(study_json, configs):
    ...
    def check_assay_graph(process_sequence_json, config):
        ...
        for process in list_of_last_processes_in_sequence:
            # build graphs backwards
            assay_graph = list()
            try:
                while True:
                    process_graph = list()
                    ...
                    protocol_id = protocols_and_types[process[
                        "executesProtocol"]["@id"]]
                    ...
                    process = [i for i in process_sequence_json if i["@id"]
                               == process["previousProcess"]["@id"]][0]
                    if process['@id'] == process["previousProcess"]["@id"]:
                        log.fatal("Previous process is same as current "
                                  "process, which forms a loop!!!!! "
                                  "Cannot find start node!!!!!!!")
                        break
            except KeyError:
                # this happens when we can"t find a previousProcess
                pass
            assay_graph.reverse()
            assay_protocol_sequence = [
                [j for j in i if not j.startswith("#")] for i in assay_graph]
            assay_protocol_sequence = [
                i for j in assay_protocol_sequence for i in j]  # flatten list
            assay_protocol_sequence_of_interest = [
                i for i in assay_protocol_sequence
                if i in config_protocol_sequence
            ]
            #  filter out protocols in sequence that are not of interest
            # (additional ones to required by config)
            squished_assay_protocol_sequence_of_interest = list()
            prev_prot = None
            for prot in assay_protocol_sequence_of_interest:
                # remove consecutive same protocols
                if prev_prot != prot:
                    squished_assay_protocol_sequence_of_interest.append(prot)
                prev_prot = prot
            from isatools.utils import contains
            if not contains(squished_assay_protocol_sequence_of_interest,
                            config_protocol_sequence):
                warnings.append({
                    "message": "Process sequence is not valid against "
                               "configuration",
                    "supplemental":
                        "Config protocol sequence {} does not in "
                        "assay protocol sequence {}".format(
                            config_protocol_sequence,
                            squished_assay_protocol_sequence_of_interest),
                    "code": 4004
                })
                log.warning("Configuration protocol sequence {} does not "
                            "match study graph found in {}".format(
                                config_protocol_sequence,
                                assay_protocol_sequence))

    protocols_and_types = dict([(
        i["@id"], i["protocolType"]["annotationValue"])
        for i in study_json["protocols"]])
...
default_config_dir = os.path.join(
    BASE_DIR, "resources", "config", "json", "default")
...
def validate(fp, config_dir=default_config_dir, log_level=None,
    ...
    try:
        ...
        check_isa_schemas(
            isa_json=isa_json, investigation_schema_path=os.path.join(
                BASE_DIR, "resources", "schemas", base_schemas_dir,
        ...
        for study_json in isa_json["studies"]:
            # Rule 1015
            check_material_ids_declared_used(study_json, get_source_ids)
            # Rule 1016
            check_material_ids_declared_used(study_json, get_sample_ids)
            # Rule 1017
            check_material_ids_declared_used(study_json, get_material_ids)
            # Rule 1018
            check_material_ids_declared_used(study_json, get_data_file_ids)
        log.info("Checking characteristic categories usage...")
        # Rules 1013 and 1022
        check_characteristic_category_ids_usage(isa_json["studies"])
        ...
        for study_json in isa_json["studies"]:
            # Rules 1009 and 1020
            check_protocol_parameter_ids_usage(study_json)
        ...
        for study_json in isa_json["studies"]:
            # Rule 1006
            check_process_sequence_links(study_json["processSequence"])
            log.info("Checking process sequences (assay)...")
            for assay_json in study_json["assays"]:
                # Rule 1006
                check_process_sequence_links(assay_json["processSequence"])
        ...
        check_isa_schemas(
            isa_json=isa_json, investigation_schema_path=os.path.join(
                config_dir, "schemas", "investigation_schema.json"))
        # if all ERRORS are resolved, then try and validate against
        # configuration
        handler.flush()
        if "(E)" in stream.getvalue():
            log.fatal("(F) There are some errors that mean validation "
                      "against configurations cannot proceed.")
            return stream
...
def batch_validate(json_file_list):
    ...
    for json_file in json_file_list:
        ...
        if not os.path.isfile(json_file):
            log.warning("Could not find ISA-JSON file, skipping {}".format(
                json_file))
        else:
    ...
    def default(self, o):
        ...
        def nulls_to_str(d):
            ...
            for k, v in d.items():
                if not isinstance(v, list) and v is None:
                    d[k] = ''
                if (k == "unit"
                    or k == "previousProcess"
                        or k == "nextProcess") and (v is None or v == ''):
                    to_del.append(k)
            for k in to_del:
                del d[k]
            return d

        # TODO: deal with non-verbose mode parsing;
        # TODO: currently will break because of missing k-v's
        clean_nulls = nulls_to_str  # creates verbose JSON if using nulls
        # to str
        # clean_nulls = remove_nulls  # optimises by removing k-v's that are
                ...
                return clean_nulls(
                    {
                        ...
                        "termSource": o.term_source.name
                        if o.term_source else None
                    }
                )
            else:
                return None
            ...
            return clean_nulls(
                {
                    ...
                    "parameters": list(map(lambda x: {
                        "@id": id_gen(x),
                        "parameterName": get_ontology_annotation(
                            x.parameter_name)
            ...
            return clean_nulls(
                {
                    ...
                    "factorValues": list(map(lambda x: clean_nulls(
                        {
                            "category": {"@id": id_gen(x.factor_name)}
                            if x.factor_name else None,
            ...
            return clean_nulls(
                {
                    ...
                    "parameterValues": list(map(
                        lambda x: get_parameter_value(x), o.parameter_values)),
                    "performer": o.performer,
                    "date": o.date,
                    "previousProcess": {
                        "@id": id_gen(o.prev_process)}
                    if o.prev_process else None,
                    "nextProcess": {
                        "@id": id_gen(o.next_process)}
                    if o.next_process else None,
                    "inputs": list(map(
                        lambda x: {"@id": id_gen(x)}, o.inputs)),
                    "outputs": list(map(
                        lambda x: {"@id": id_gen(x)}, o.outputs)),
                    "comments": get_comments(o.comments)
                }
            )
        ...
        def get_parameter_value(o):
            return clean_nulls(
                {
                    "category": {
                        "@id": id_gen(o.category)} if o.category else None,
                    "value": get_value(o.value),
                    "unit": {"@id": id_gen(o.unit)} if o.unit else None
                }
            )
            ...
            return clean_nulls(
                {
                    "measurementType": get_ontology_annotation(
                        o.measurement_type),
                    "technologyType": get_ontology_annotation(
                        o.technology_type),
                    ...
                    "dataFiles": list(map(
                        lambda x: get_data_file(x), o.data_files)),
            ...
            return clean_nulls(
                {
                    ...
                    "description": o.description,
                    "comments": get_comments(o.comments),
                    "ontologySourceReferences": list(map(
                        lambda x: get_ontology_source(x),
                        o.ontology_source_references)),
                    "people": get_people(o.contacts),
                    "publicReleaseDate": o.public_release_date,
                    "submissionDate": o.submission_date,
                    "publications": get_publications(o.publications),
