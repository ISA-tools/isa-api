...
class Characteristic(Commentable):
    ...
    def __repr__(self):
        ...
        ).format(characteristic=self, category=repr(self.category), value=repr(self.value), unit=repr(self.unit))

    def __str__(self):
        value = (
            self.value if isinstance(self.value, OntologyAnnotation) else self.value if self.value is not None else ""
        )
        unit = (
            self.unit.term if isinstance(self.unit, OntologyAnnotation) else self.unit if self.unit is not None else ""
        )
        return (
            "Characteristic(\n\t"
            "category={category}\n\t"
    ...
    def __eq__(self, other):
        return (
            isinstance(other, Characteristic)
