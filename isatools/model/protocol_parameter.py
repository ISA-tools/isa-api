...
class ProtocolParameter(Commentable, Identifiable):
    ...
    def parameter_name(self, val):
        if val is None or isinstance(val, OntologyAnnotation):
            ...
            error_msg = (
                "ProtocolParameter.parameter_name must be either a string or an OntologyAnnotation or None; got {0}:{1}"
            ).format(val, type(val))
            raise AttributeError(error_msg)

    def __repr__(self):
        return (
            "isatools.model.ProtocolParameter(parameter_name={parameter_name}, comments={parameter.comments})"
        ).format(parameter=self, parameter_name=repr(self.parameter_name))
    ...
    def __eq__(self, other):
        return (
            isinstance(other, ProtocolParameter)
    ...
    def to_dict(self, ld=False):
        protocol_parameter = {"@id": self.id, "parameterName": self.parameter_name.to_dict(ld=ld)}
