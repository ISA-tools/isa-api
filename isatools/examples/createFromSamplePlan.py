...
def create_descriptor():
    ...
    treatment_factory.add_factor_value(f1, {"aspirin", "ibuprofen", "paracetamol"})
    treatment_factory.add_factor_value(f2, {"low", "medium", "high"})
    treatment_factory.add_factor_value(f3, {"short", "long"})
    fullfactorial_design_treatments = treatment_factory.compute_full_factorial_design()
    ...
    study = StudyDesignFactory(plan, treatment_sequence).create_study_from_plan()
