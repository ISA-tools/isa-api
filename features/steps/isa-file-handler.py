...
from isatools.net.storage_adapter import CONTENTS, REPOS, IsaGitHubStorageAdapter
...
def step_impl_03(context):
    ...
    httpretty.register_uri(
        httpretty.POST, auth_url, body=json.dumps(auth_res_body), content_type="application/json", status=201
    )
...
def step_impl_09(context):
    ...
    with open(fixture_file_path) as json_file:
        ...
        download_url = "/".join(
            [GITHUB_API_URL, REPOS, context.owner_name, context.repo_name, CONTENTS, context.source_path]
        )
...
def step_impl_11(context):
    ...
    fixture_file_name = (
        "_".join([context.owner_name, context.repo_name, context.source_path])
        .replace("/", "_")
        .replace(" ", "_")
        .replace(".zip", ".json")
    )
...
def step_impl_13(context):
    ...
    fixture_file_path_encoded = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "fixtures", fixture_file_name_encoded)
    )
    fixture_file_path_raw = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "..", "fixtures", fixture_file_name_raw)
    )

    # create the url to GET the encoded dataset
    encoded_file_url = "/".join(
        [GITHUB_API_URL, REPOS, context.owner_name, context.repo_name, CONTENTS, context.source_path]
    )
...
def step_impl_18(context):
    ...
    with open(fixture_file_path) as json_file:
        ...
        httpretty.register_uri(
            httpretty.GET, encoded_file_url, body=json.dumps(context.text_encoded), content_type="text/plain"
        )
...
def step_impl_21(context):
    ...
    :type context: behave.runner.Context
    """
    context.not_found_payload = {"message": "Not Found", "documentation_url": "https://developer.github.com/v3"}
...
def step_impl_22(context):
    ...
    """
    httpretty.register_uri(httpretty.GET, context.download_url, body=json.dumps(context.not_found_payload), status=404)
    expect(context.isa_adapter.retrieve).when.called_with(
        context.source_path,
        destination=context.destination_path,
        owner=context.owner_name,
        repository=context.repo_name,
