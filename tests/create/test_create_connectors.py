...
from isatools.create.constants import DATA_FILE, DEFAULT_EXTENSION, IS_TREATMENT_EPOCH, SEQUENCE_ORDER_FACTOR_
from isatools.create.model import AssayGraph, SampleAndAssayPlan, StudyArm, StudyCell, StudyDesign
from isatools.isajson import ISAJSONEncoder
from isatools.model import Characteristic, Investigation, OntologyAnnotation, OntologySource, Sample, Study
from isatools.tests.create_sample_assay_plan_odicts import annotated_ms_assay_dict, ms_assay_dict
...
class TestMappings(unittest.TestCase):
    def setUp(self):
        ...
        for filename in filenames:
            file_path = os.path.abspath(
                os.path.join(os.path.dirname(__file__), "..", "data", "json", "create", "templates", filename)
            )
    ...
    def test_map_ontology_annotation(self):
        ...
        self.assertEqual(
            _map_ontology_annotation(source, False),
            OntologyAnnotation(
                term="toto",
                term_accession="GO:2314",
                term_source=OntologySource(name="A", file="file", version="1", description="toto"),
            ),
        )
    ...
    def _load_config(file_name):
        ds_design_config_file_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "data", "json", "create", "datascriptor", file_name)
        )
    ...
    def test_generate_assay_ord_dict_from_datascriptor_config_fail_missing_param_value(self):
        ...
        self.assertRaises(
            ValueError,
            generate_assay_ord_dict_from_config,
            assay_config_with_empty_param_value,
            test_arm_name,
            test_epoch_no,
        )
    ...
    def test_generate_assay_ord_dict_from_config_file_extension(self):
        ...
        assay_odict = generate_assay_ord_dict_from_config(marker_panel_assay_plan_config, arm_name="Arm_0", epoch_no=1)
        assay_graph = AssayGraph.generate_assay_plan_from_dict(assay_odict)
        self.assertIsInstance(assay_graph, AssayGraph)
    ...
    def test_generate_study_design(self):
        ...
        for sample in study.samples:
            ...
            try:
                ...
                continue
        ms_assay = next(assay for assay in study.assays if assay.filename.endswith("mass-spectrometry.txt"))
        self.assertTrue(all(data_file.filename.split(".")[-1] == "mzML" for data_file in ms_assay.data_files))
    ...
    def test_generate_study_design_with_chained_protocols_and_ontology_annotations(self):
        ...
        self.assertTrue(all(data_file.filename.split(".")[-1] == "raw" for data_file in sequencing_assay.data_files))
        ...
        marker_panel_samples = [
            proc.inputs[0]
            for proc in marker_panel_assay.process_sequence
        ...
        self.assertTrue(
            all(
                sample.characteristics[0].value.term_accession == blood_sample["iri"] for sample in marker_panel_samples
            )
        )
        self.assertTrue(all(data_file.filename.split(".")[-1] == "raw" for data_file in marker_panel_assay.data_files))
        if SLOW_TESTS:
