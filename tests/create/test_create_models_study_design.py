...
class NonTreatmentTest(unittest.TestCase):
    ...
    def test_init_and_properties(self):
        ...
        self.assertEqual(
            self.non_treatment.duration,
            FactorValue(factor_name=DURATION_FACTOR, value=self.DURATION_VALUE, unit=self.DURATION_UNIT),
        )
    ...
    def test_ne(self):
        other_non_treatment = NonTreatment(duration_value=self.OTHER_DURATION_VALUE, duration_unit=self.DURATION_UNIT)
...
class TreatmentTest(unittest.TestCase):
    ...
    def test_element(self):
        ...
        self.assertEqual(
            er_msg.exception.args[0], "intervention_type must be string or OntologyAnnotation. -1 was provided."
        )
...
class StudyCellTest(unittest.TestCase):
    def setUp(self):
        ...
        self.third_treatment = Treatment(
            factor_values=(
                FactorValue(factor_name=BASE_FACTORS[0], value=FACTORS_0_VALUE_ALT),
                FactorValue(factor_name=BASE_FACTORS[1], value=FACTORS_1_VALUE, unit=FACTORS_1_UNIT),
                FactorValue(factor_name=BASE_FACTORS[2], value=FACTORS_2_VALUE_ALT, unit=FACTORS_2_UNIT),
        ...
        self.fourth_treatment = Treatment(
            factor_values=(
        ...
        self.screen = NonTreatment(
            element_type=SCREEN, duration_value=SCREEN_DURATION_VALUE, duration_unit=DURATION_UNIT
        )
        self.run_in = NonTreatment(
            element_type=RUN_IN, duration_value=WASHOUT_DURATION_VALUE, duration_unit=DURATION_UNIT
        )
        self.washout = NonTreatment(
            element_type=WASHOUT, duration_value=WASHOUT_DURATION_VALUE, duration_unit=DURATION_UNIT
        )
        self.follow_up = NonTreatment(
            element_type=FOLLOW_UP, duration_value=FOLLOW_UP_DURATION_VALUE, duration_unit=DURATION_UNIT
        )
        self.potential_concomitant_washout = NonTreatment(
            element_type=WASHOUT, duration_value=FACTORS_2_VALUE, duration_unit=FACTORS_2_UNIT
        )
    ...
    def test_elements_not_of_type(self):
        ...
        with self.assertRaises(
            AttributeError, msg="'elements must be an Element, a list of Elements, or a tuple of Elements'"
        ) as er_msg:
            self.assertEqual(
                er_msg.exception.args[0],
                self.elements(cell_element),
    ...
    def test_insert_element(self):
        element = ""
        with self.assertRaises(
            AttributeError, msg="'element must be either an Element or a set of treatments'"
        ) as er_msg:
            self.assertEqual(
                er_msg.exception.args[0],
                self.cell.insert_element(element, 1),
    ...
    def test_non_treatment_check__empty_cell_00(self):
        self.assertTrue(
            self.cell._non_treatment_check([], self.screen), "A SCREEN element can always be added to an empty cell"
        )

    def test_non_treatment_check__empty_cell_01(self):
        self.assertTrue(
            self.cell._non_treatment_check([], self.run_in), "A RUN-IN element can always be added to an empty cell"
        )

    def test_non_treatment_check__empty_cell_02(self):
        self.assertTrue(
            self.cell._non_treatment_check([], self.washout), "A WASHOUT element can always be added to an empty cell"
        )

    def test_non_treatment_check__empty_cell_03(self):
        self.assertTrue(
            self.cell._non_treatment_check([], self.follow_up),
            "A FOLLOW-UP element can always be added to an empty cell",
        )

    def test_non_treatment_check__screen_cell_00(self):
        self.assertFalse(
            self.cell._non_treatment_check([self.screen], self.screen),
            "A SCREEN element cannot be added to a cell with a SCREEN",
        )

    def test_non_treatment_check__screen_cell_01(self):
        self.assertTrue(
            self.cell._non_treatment_check([self.screen], self.run_in),
            "A RUN-IN element can be added to a cell with a SCREEN after the SCREEN",
        )

    def test_non_treatment_check__screen_cell_02(self):
        self.assertFalse(
            self.cell._non_treatment_check([self.screen], self.run_in, 0),
    ...
    def test_non_treatment_check__follow_up_cell_03(self):
        self.assertFalse(
            self.cell._non_treatment_check([self.follow_up], self.follow_up),
            "A FOLLOW-UP element cannot be added to a cell with a FOLLOW-UP",
        )

    def test_non_treatment_check__treatment_cell_00(self):
        self.assertFalse(
            self.cell._non_treatment_check([self.first_treatment], self.screen),
            "A SCREEN element cannot be added to a cell with a treatment",
        )
        self.assertFalse(
            self.cell._non_treatment_check([self.first_treatment], self.screen, 0),
    ...
    def test_non_treatment_check__treatment_cell_01(self):
        self.assertFalse(
            self.cell._non_treatment_check([self.first_treatment], self.run_in),
            "A RUN-IN element can be added to a cell with a treatment",
        )
        self.assertFalse(
            self.cell._non_treatment_check([self.first_treatment], self.run_in, 0),
    ...
    def test_non_treatment_check__treatment_cell_02(self):
        self.assertTrue(
            self.cell._non_treatment_check([self.first_treatment], self.washout),
            "A WASHOUT element can be added to a cell with a treatment after the treatment",
        )
        self.assertTrue(
            self.cell._non_treatment_check([self.first_treatment], self.washout, 0),
    ...
    def test_non_treatment_check__treatment_cell_03(self):
        self.assertFalse(
            self.cell._non_treatment_check([self.first_treatment], self.follow_up),
            "A FOLLOW-UP element cannot be added to a cell with a treatment",
        )
        self.assertFalse(
            self.cell._non_treatment_check([self.first_treatment], self.follow_up, 0),
    ...
    def test_non_treatment_check__multi_element_cell_00(self):
        self.assertTrue(
            self.cell._non_treatment_check([self.first_treatment, self.washout, self.second_treatment], self.washout),
            "A WASHOUT element can be added at the end of a cell with 2 treatments interspersed by a washout",
        )
        self.assertTrue(
            self.cell._non_treatment_check([self.first_treatment, self.washout, self.second_treatment], self.washout),
            "A WASHOUT element can be added at the beginning of a cell with 2 treatments interspersed by a washout",
        )

    def test_non_treatment_check_multi_element_cell_01(self):
        self.assertTrue(
            self.cell._non_treatment_check(
                [{self.first_treatment, self.second_treatment}, self.washout, self.second_treatment], self.washout
            ),
            "A WASHOUT element can be added at the end of a cell with a treatment set and a treatment "
            "interspersed by a washout",
        )
        self.assertTrue(
            self.cell._non_treatment_check(
                [{self.first_treatment, self.second_treatment}, self.washout, self.second_treatment], self.washout, 0
            ),
            "A WASHOUT element can be added at the beginning of a cell with a treatment set and a "
    ...
    def test_non_treatment_check_multi_element_cell_02(self):
        self.assertTrue(
            self.cell._non_treatment_check(
                [self.first_treatment, self.washout, {self.fourth_treatment, self.second_treatment}], self.washout
            ),
            "A WASHOUT element can be added at the end of a cell with a treatment and a treatment set"
            "interspersed by a washout",
        )
        self.assertTrue(
            self.cell._non_treatment_check(
                [self.first_treatment, self.washout, {self.fourth_treatment, self.second_treatment}], self.washout, 0
            ),
            "A WASHOUT element can be added at the beginning of a cell with a treatment and a "
    ...
    def test_treatment_check__washout_cell(self):
        self.assertTrue(self.cell._treatment_check([self.washout]), "A treatment can be inserted into a WASHOUT cell")

    def test_treatment_check__follow_up_cell(self):
        self.assertFalse(
            self.cell._treatment_check([self.follow_up]), "A treatment cannot be inserted into a FOLLOW-UP cell"
        )

    def test_treatment_check__treatment_cell_00(self):
        self.assertTrue(
            self.cell._treatment_check([self.first_treatment]),
            "A treatment can be inserted into a cell with a treatment",
        )

    def test_treatment_check__treatment_cell_01(self):
        self.assertTrue(
            self.cell._treatment_check([self.first_treatment, self.second_treatment]),
            "A treatment can be inserted into a cell with two treatment",
        )

    def test_treatment_check__treatment_cell_02(self):
        self.assertTrue(
            self.cell._treatment_check([self.first_treatment, {self.second_treatment, self.third_treatment}]),
    ...
    def test_treatment_check__treatment_cell_03(self):
        self.assertTrue(
            self.cell._treatment_check(
                [self.first_treatment, self.washout, {self.second_treatment, self.third_treatment}, self.washout]
            ),
            "A treatment can be inserted into a cell with a treatment a concomitant treatment and two washouts",
        )

    # _concomitant_treatment_check() tests
    def test_concomitant_treatments_check_00(self):
        self.assertTrue(
            self.cell._concomitant_treatments_check({self.first_treatment, self.second_treatment}),
    ...
    def test_concomitant_treatments_check_01(self):
        self.assertFalse(
            self.cell._concomitant_treatments_check(
                {self.first_treatment, self.second_treatment, self.potential_concomitant_washout}
            ),
    ...
    def test_concomitant_treatments_check_02(self):
        self.assertTrue(
            self.cell._concomitant_treatments_check(
                {self.first_treatment, self.second_treatment, self.fourth_treatment}
            ),
            "Concomitant treatment must have same duration (no semantic reasoning on units)",
        )
    ...
    def test_concomitant_treatments_check_03(self):
        self.assertFalse(
            self.cell._concomitant_treatments_check(
                {self.first_treatment, self.second_treatment, self.third_treatment}
            ),
            "Concomitant treatment must have same duration (no semantic reasoning on units)",
        )
    ...
    def test_insert_element_screen(self):
        ...
        self.assertRaises(
            ValueError, self.cell.insert_element, self.screen, "A SCREEN cannot be added to a a cell with a SCREEN"
        )
        self.assertEqual(self.cell.elements, [self.screen])
        self.assertRaises(
            ValueError,
            self.cell.insert_element,
            self.first_treatment,
            "A treatment cannot be added to a cell with a SCREEN",
        )
        self.assertEqual(self.cell.elements, [self.screen])
        self.assertRaises(
            ValueError, self.cell.insert_element, self.follow_up, "A FOLLOW-UP cannot ba added to a cell with a SCREEN"
        )
        self.assertEqual(self.cell.elements, [self.screen])
        self.assertRaises(
            ValueError,
        ...
        self.assertEqual(
            self.cell.elements, [self.screen, self.run_in], "A RUN-IN is inserted in the cell after theSCREEN"
        )
    ...
    def test_insert_element_run_in(self):
        ...
        self.assertEqual(self.cell.elements, [self.run_in])
        self.assertRaises(
            ValueError, self.cell.insert_element, self.follow_up, "A FOLLOW-UP cannot ba added to a cell with a RUN-IN"
        )
        self.assertEqual(self.cell.elements, [self.run_in])
        self.assertRaises(
            ValueError,
        ...
        self.assertEqual(
            self.cell.elements, [self.screen, self.run_in], "A SCREEN is inserted in the cell before theRUN-IN"
        )
    ...
    def test_insert_element_washout__00(self):
        ...
        self.assertRaises(
            ValueError, self.cell.insert_element, self.screen, "A SCREEN cannot be added to a a cell with a WASHOUT"
        )
        self.assertEqual(self.cell.elements, [self.washout])
        self.assertRaises(
            ValueError, self.cell.insert_element, self.run_in, "A RUN-IN cannot be added to a cell with a WASHOUT"
        )
        self.assertEqual(self.cell.elements, [self.washout])
        self.assertRaises(
            ValueError, self.cell.insert_element, self.follow_up, "A FOLLOW-UP cannot ba added to a cell with a WASHOUT"
        )
        self.assertEqual(self.cell.elements, [self.washout])
        self.cell.insert_element({self.first_treatment, self.fourth_treatment})
        self.assertEqual(
            self.cell.elements,
            [self.washout, {self.first_treatment, self.fourth_treatment}],
    ...
    def test_insert_element_washout__01(self):
        ...
        self.assertEqual(
            self.cell.elements, [self.washout, self.second_treatment], "A treatment is added after the WASHOUT"
        )
    ...
    def test_insert_element_follow_up(self):
        ...
        self.assertRaises(
            ValueError, self.cell.insert_element, self.screen, "A SCREEN cannot be added to a a cell with a FOLLOW-UP"
        )
        self.assertEqual(self.cell.elements, [self.follow_up])
        self.assertRaises(
            ValueError,
        ...
        self.assertRaises(
            ValueError, self.cell.insert_element, self.run_in, "A RUN-IN cannot be added to a cell with a FOLLOW-UP"
        )
    ...
    def test_insert_element_treatment(self):
        ...
        self.assertEqual(
            self.cell.elements,
            [self.first_treatment, self.second_treatment],
    ...
    def test_insert_element_concomitant_treatment(self):
        ...
        self.assertEqual(
            self.cell.elements,
            [{self.first_treatment, self.second_treatment}, self.second_treatment],
        ...
        self.assertEqual(
            ...
            "A washout can be added to a cell with a treatment set and a treatment, at the end",
        )
        self.assertRaises(
            ValueError,
            self.cell.insert_element,
    ...
    def test_contains_non_treatment_by_type_empty_cell(self):
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(SCREEN), False, "An empty cell contains no SCREEN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(RUN_IN), False, "An empty cell contains no RUN-IN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(WASHOUT), False, "An empty cell contains no WASHOUT"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(FOLLOW_UP), False, "An empty cell contains no FOLLOW_UP"
        )

    def test_contains_non_treatment_by_type_screen_cell(self):
        self.cell.elements = [self.screen]
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(SCREEN), True, "A SCREEN cell contains a SCREEN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(RUN_IN), False, "A SCREEN cell contains no RUN-IN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(WASHOUT), False, "A SCREEN cell contains no WASHOUT"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(FOLLOW_UP), False, "A SCREEN cell contains no FOLLOW_UP"
        )

    def test_contains_non_treatment_by_type_run_in_cell(self):
        self.cell.elements = [self.run_in]
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(SCREEN), False, "A RUN-IN cell contains no SCREEN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(RUN_IN), True, "A RUN-IN cell contains a RUN-IN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(WASHOUT), False, "A RUN-IN cell contains no WASHOUT"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(FOLLOW_UP), False, "A RUN-IN cell contains no FOLLOW_UP"
        )

    def test_contains_non_treatment_by_type_washout_cell(self):
        self.cell.elements = [self.washout]
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(SCREEN), False, "A WASHOUT cell contains no SCREEN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(RUN_IN), False, "A WASHOUT cell contains no RUN-IN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(WASHOUT), True, "A WASHOUT cell contains a WASHOUT"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(FOLLOW_UP), False, "A WASHOUT cell contains no FOLLOW_UP"
        )

    def test_contains_non_treatment_by_type_follow_up_cell(self):
        self.cell.elements = [self.follow_up]
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(SCREEN), False, "A FOLLOW-UP cell contains no SCREEN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(RUN_IN), False, "A FOLLOW-UP cell contains no RUN-IN"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(WASHOUT), False, "A FOLLOW-UP cell contains a WASHOUT"
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(FOLLOW_UP), True, "A FOLLOW-UP cell contains no FOLLOW_UP"
        )

    def test_contains_non_treatment_by_type_single_treatment_cell(self):
        self.cell.elements = [self.first_treatment]
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(SCREEN),
            False,
            "A single treatment cell contains no SCREEN",
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(RUN_IN),
            False,
            "A single treatment cell contains no RUN-IN",
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(WASHOUT),
            False,
            "A single treatment cell contains a WASHOUT",
        )
        self.assertEqual(
            self.cell.contains_non_treatment_element_by_type(FOLLOW_UP),
            False,
    ...
    def test_get_all_elements_00(self):
        ...
        self.assertEqual(
            self.cell.get_all_elements(),
            [self.first_treatment, self.washout, self.second_treatment, self.fourth_treatment, self.washout],
        )
    ...
    def test_get_all_elements_01(self):
        ...
        self.assertEqual(
            set(self.cell.get_all_elements()),
            {self.first_treatment, self.washout, self.second_treatment, self.fourth_treatment},
        )
...
class ProtocolNodeTest(unittest.TestCase):
    ...
    def test_properties(self):
        ...
        test_parameter_values = [
            ...
            ParameterValue(
                category=ProtocolParameter(parameter_name="another test param"),
                value=12,
                unit=OntologyAnnotation(term="z"),
            ),
        ]
        ...
        with self.assertRaises(
            AttributeError, msg="The 'parameters' property cannot be set directly. Set parameter_values instead."
        ) as er_msg:
            param = -1
            node.parameters = param
        self.assertEqual(
            er_msg.exception.args[0], "The 'parameters' property cannot be set directly. Set parameter_values instead."
        )
...
class ProductNodeTest(unittest.TestCase):
    ...
    def test_add_characteristics(self):
        ...
        self.assertEqual(
            node.characteristics[0].category,
            OntologyAnnotation(term="toto", term_source=None, term_accession="", comments=[]),
        )
    ...
    def test_add_characteristics_of_wrong_type(self):
        ...
        with self.assertRaises(
            TypeError,
            msg="A characteristic must be either a string or a Characteristic,"
...
class QualityControlSampleTest(unittest.TestCase):
    ...
    def test_init(self):
        qc_sample = QualityControlSample(
            characteristics=self.sample_characteristic, name="qc_sample_test", qc_sample_type=QC_SAMPLE_TYPE_PRE_RUN
        )
    ...
    def test_properties(self):
        qc_sample = QualityControlSample(
            characteristics=self.sample_characteristic,
            name="qc_sample_test",
...
class QualityControlTest(unittest.TestCase):
    ...
    def test_str(self):
        ...
        )""".format(self.pre_run_sample_type.id, self.post_run_sample_type.id, self.dummy_sample_type.id, 20).replace(
                " ", ""
            ),
        )
...
class AssayGraphTest(unittest.TestCase):
    ...
    def test_node_attributes(self):
        ...
        with self.assertRaises(
            AttributeError, msg="ProductNode name must be a string, -1 supplied of type <class 'int'>"
        ) as er_msg:
            ProductNode(name=-1)
        self.assertEqual(
            er_msg.exception.args[0], "ProductNode name must be a string, -1 supplied of type <class 'int'>"
        )

        with self.assertRaises(
            AttributeError, msg="ProductNode size must be a natural number, i.e integer >= 0"
        ) as er_msg:
        ...
        with self.assertRaises(
            AttributeError,
            msg="The provided ProductNode is not one of the allowed values: {'labeled extract', 'sample', 'source', 'extract', 'data file'}",
        ) as er_msg:
            BAD = "bad_type"
    ...
    def test_add_nodes_and_links_success(self):
        ...
        links = [
            (self.sample_node, self.protocol_node_rna),
            (self.protocol_node_rna, self.mrna_node),
            (self.protocol_node_rna, self.mirna_node),
        ]
    ...
    def test_next_nodes(self):
        ...
        self.assertEqual(assay_graph.next_nodes(self.protocol_node_rna), {self.mirna_node, self.mrna_node})
    ...
    def test_as_networkx_graph(self):
        ...
        self.assertEqual(set(nx_graph.nodes), {node.id for node in self.assay_graph.nodes})
        self.assertEqual(nx_graph.edges, {(u.id, v.id) for u, v in self.assay_graph.links})
    ...
    def test_eq(self):
        ...
        first_plan = AssayGraph(
            id_="assay-graph/00", measurement_type="genomic extraction", technology_type="nucleic acid extraction"
        )
    ...
    def test_repr(self):
        ...
        first_graph = AssayGraph(
            id_="assay-graph-01", measurement_type="genomic extraction", technology_type="nucleic acid extraction"
        )
        first_graph.add_nodes(nodes)
        first_graph.add_links(links)
        second_graph = AssayGraph(
            id_="assay-graph-01", measurement_type="genomic extraction", technology_type="nucleic acid extraction"
        )
...
class SampleAndAssayPlanTest(unittest.TestCase):
    def setUp(self):
        ...
        self.metabolomic_assay_graph = AssayGraph(
            id_="assay-graph/01", measurement_type="metabolomic analysis", technology_type="mass spectrometry"
        )
    ...
    def test_add_element_to_map_success(self):
        ...
        self.assertEqual(plan.sample_to_assay_map, {self.blood_node: {self.genomic_assay_graph}})
        plan.add_element_to_map(self.tissue_node, self.genomic_assay_graph)
        self.assertEqual(
            plan.sample_to_assay_map,
            {self.blood_node: {self.genomic_assay_graph}, self.tissue_node: {self.genomic_assay_graph}},
        )
    ...
    def test_from_sample_and_assay_plan_dict_no_validation(self):
        ...
        self.assertEqual(
            len(list(filter(lambda node: node.name.endswith("mass spectrometry"), ms_assay_graph.nodes))), 4
        )
        self.assertEqual(len(list(filter(lambda node: node.name == "raw spectral data file", ms_assay_graph.nodes))), 4)
    ...
    def test_study_sample_plan_repr(self):
        ...
        self.assertEqual(
            repr(self.plan),
            """isatools.create.model.SampleAndAssayPlan(name=test plan, sample_plan=[], assay_plan=set(), sample_to_assay_map={})""",
        )
...
class StudyArmTest(unittest.TestCase):
    def setUp(self):
        ...
        self.cell_concomitant_treatments = StudyCell(
            "CONCOMITANT TREATMENTS", elements=([{self.second_treatment, self.fourth_treatment}])
        )
        ...
        self.cell_multi_elements = StudyCell(
            "MULTI ELEMENTS",
            elements=[
                {self.first_treatment, self.second_treatment, self.fourth_treatment},
                self.washout,
        ...
        self.cell_multi_elements_padded = StudyCell(
            "MULTI ELEMENTS PADDED",
            elements=[
                self.first_treatment,
                self.washout,
                {self.second_treatment, self.fourth_treatment},
    ...
    def test_add_item_to_arm__single_unit_cells_00(self):
        ...
        with self.assertRaises(
            ValueError, msg="Another cell containing a screen cannot be added to the StudyArm"
        ) as ex_cm:
        ...
        with self.assertRaises(
            ValueError, msg="Another cell containing a run-in cannot be added to the StudyArm"
        ) as ex_cm:
        ...
        with self.assertRaises(
            ValueError, msg="Another cell containing a WASHOUT cannot be added to the StudyArm"
        ) as ex_cm:
        ...
        self.assertEqual(
            cells[6], self.cell_concomitant_treatments, "The concomitant treatments cell has been added to the arm"
        )
    ...
    def test_add_item_to_arm__multi_unit_cells_00(self):
        self.arm.add_item_to_arm_map(self.cell_screen_and_run_in, None)
        with self.assertRaises(
            ValueError, msg="A cell beginning with a WASHOUT element cannot be added to aan ARM ending with a RUN-IN"
        ) as ex_cm:
    ...
    def test_add_item_to_arm__multi_unit_cells_01(self):
        self.arm.add_item_to_arm_map(self.cell_screen, None)
        with self.assertRaises(
            ValueError, msg="A cell beginning with a FOLLOW-UP element cannot be added to aan ARM ending with a SCREEN"
        ) as ex_cm:
        ...
        self.assertEqual(len(cells), 2, "One mapping has been added to the arm")
        self.assertEqual(
            cells[1], self.cell_multi_elements_padded, "The multi-step treatment cell has been added to the arm"
        )
        self.assertEqual(plans[1], self.sample_assay_plan, "There is a sample plan for this specific cell")
        self.arm.add_item_to_arm_map(self.cell_follow_up, self.sample_assay_plan)
        cells, plans = zip(*self.arm.arm_map.items())
    ...
    def test_add_item_to_arm__follow_up_to_empty_cell(self):
        with self.assertRaises(
            ValueError, msg="A cell beginning with a FOLLOW-UP element cannot be added to an empty arm."
        ) as ex_cm:
            self.arm.add_item_to_arm_map(self.cell_follow_up, self.sample_assay_plan)
    ...
    def test_source_type_property_fail(self):
        ...
        self.assertEqual(
            ex_cm.exception.args[0],
            "The source_type property must be either a string or a Characteristic. 128 was supplied.",
        )
    ...
    def test_source_characteristics_fail(self):
        ...
        test_characteristics = [Characteristic(category="sex", value="M"), "age group - old"]
        with self.assertRaises(AttributeError, msg="source_characteristics can only contain Characteristic"):
            self.arm.source_characteristics = test_characteristics
    ...
    def test_group_size_property_fail_00(self):
        with self.assertRaises(AttributeError, msg="Only positive integers can be assigned to group_size") as ex_cm:
    ...
    def test_arm_map_property_success_00(self):
        self.assertEqual(
            self.arm.arm_map, OrderedDict(), "The ordered mapping StudyCell -> SampleAndAssayPlan is empty."
        )
        ...
        self.assertEqual(
            self.arm.arm_map,
            ord_dict,
            "The ordered mapping StudyCell -> SampleAndAssayPlan has been correctly set for single-treatment cells.",
        )
    ...
    def test_arm_map_property_fail_wrong_type(self):
        with self.assertRaises(
            AttributeError, msg="An error is raised if an object of the wrong type is provided to the assignment."
        ) as ex_cm:
    ...
    def test_arm_map_property_fail_wrong_value_00(self):
        with self.assertRaises(
            AttributeError, msg="An error is raised if an object of the wrong value is provided to the assignment."
        ) as ex_cm:
            ord_dict = OrderedDict(
                [
                    (self.cell_screen, None),
    ...
    def test_study_arm_repr(self):
        self.assertEqual(
            repr(self.arm),
            """isatools.create.model.StudyArm(name=test arm 0, source_type=Characteristic(
...
class BaseStudyDesignTest(unittest.TestCase):
    def setUp(self):
        ...
        self.third_arm_no_run_in = StudyArm(
            name=TEST_STUDY_ARM_NAME_02,
            group_size=20,
            arm_map=OrderedDict(
                [
        ...
        self.arm_same_name_as_third = StudyArm(
            name=TEST_STUDY_ARM_NAME_02,
            group_size=10,
            arm_map=OrderedDict(
                [
        ...
        self.pre_run_sample_type = ProductNode(
            ...
            name="water",
            size=5,
            characteristics=(Characteristic(category="dilution", value=10, unit="mg/L"),),
        )
...
class StudyDesignTest(BaseStudyDesignTest):
    ...
    def test_init(self):
        self.assertIsInstance(
            getattr(self.study_design, "_StudyDesign__name", None), str, "The __name has been initialized as a string"
        )
        self.assertEqual(
            getattr(self.study_design, "_StudyDesign__study_arms", None),
            set(),
    ...
    def test_description_property(self):
        ...
        with self.assertRaises(
            AttributeError, msg="'The value assigned to 'description' must be text (i.e. string)'"
        ) as er_msg:
            self.study_design.description = wrong_study_description
            self.assertEqual(
                er_msg.exception.args[0],
                self.study_design.description,
                "The value assigned to 'description' must be text (i.e. string)",
            )
    ...
    def test_design_type_property(self):
        ...
        with self.assertRaises(
            AttributeError, msg="'The value assigned to 'design_type' must be a string or OntologyAnnotation'"
        ) as er_msg:
            self.__design_type = wrong_study_design_type
            self.assertEqual(
                er_msg.exception.args[0],
                self.__design_type,
                "The value assigned to 'design_type' must be a string or OntologyAnnotation",
            )
    ...
    def test_source_type_property(self):
        ...
        wrong_source_type = 1
        with self.assertRaises(
            AttributeError, msg="'A characteristic must be either a string or a Characteristic, {0} supplied'"
        ) as er_msg:
            self.__source_type = wrong_source_type
            self.assertEqual(
                er_msg.exception.args[0],
                self.__source_type,
                "A characteristic must be either a string or a Characteristic, {0} supplied",
            )

    def test_study_arms_property(self):
        study_arms = ["study_arm"]
        with self.assertRaises(
            AttributeError, msg="'The value assigned to 'study_arms' must be an iterable'"
        ) as er_msg:
            self.study_design.study_arms = study_arms
            self.assertEqual(
                er_msg.exception.args[0], isinstance(study_arms, Iterable), "Not a valid study arm: wrong type of arm"
            )
        ...
        with self.assertRaises(AttributeError, msg="Not a valid study arm: wrong type of arm") as er_msg:
            ...
            self.assertEqual(
                er_msg.exception.args[0], self.study_design.study_arms, "Not a valid study arm: wrong type of arm"
            )

    def test_add_study_arm_00(self):
        self.study_design.add_study_arm(self.first_arm)
        self.assertIn(
            self.first_arm, self.study_design.study_arms, "The Study Arm has been correctly added to the StudyDesign"
        )

    def test_add_study_arm_01(self):
        self.study_design.add_study_arm(self.third_arm)
        self.assertIn(
            self.third_arm, self.study_design.study_arms, "The Study Arm has been correctly added to the StudyDesign"
        )
        self.study_design.add_study_arm(self.second_arm)
        self.assertIn(
            self.second_arm, self.study_design.study_arms, "The Study Arm has been correctly added to the StudyDesign"
        )
        with self.assertRaises(ValueError, msg="An integer cannot be assigned as StudyDesign name") as ex_cm:
    ...
    def test_add_study_arm_02(self):
        with self.assertRaises(TypeError, msg="A Treatment cannot be added to a StudyDesign, only StudyArms") as ex_cm:
    ...
    def test_treatments_property_01(self):
        ...
        treatment_set = self.study_design.treatments
        self.assertEqual(
            treatment_set, {self.first_treatment, self.second_treatment, self.third_treatment, self.fourth_treatment}
        )
    ...
    def test_generate_isa_study_single_arm_single_cell_elements(self):
        ...
        expected_num_of_samples = (
            reduce(
                lambda acc_value, sample_node: acc_value + sample_node.size, self.nmr_sample_assay_plan.sample_plan, 0
            )
            * single_arm.group_size
            * len([a_plan for a_plan in single_arm.arm_map.values() if a_plan is not None])
        )
        ...
        extraction_processes = [
            process
            for process in treatment_assay.process_sequence
        ...
        self.assertEqual(
            len(nmr_processes), 8 * nmr_assay_dict["nmr spectroscopy"]["#replicates"] * expected_num_of_samples
        )
        self.assertEqual(
        ...
        log.debug(
            "Process sequence: {0}".format(
                [
                    (
                        process.name,
                        getattr(process.prev_process, "name", None),
                        getattr(process.next_process, "name", None),
                    )
                    for process in treatment_assay.process_sequence
                ]
            )
    ...
    def test_generate_isa_study_two_arms_single_cell_elements(self):
        ...
        expected_num_of_samples_nmr_plan_first_arm = (
            reduce(
                lambda acc_value, sample_node: acc_value + sample_node.size, self.nmr_sample_assay_plan.sample_plan, 0
            )
            * first_arm.group_size
        )
        expected_num_of_samples_ms_plan_first_arm = (
            reduce(
                lambda acc_value, sample_node: acc_value + sample_node.size, self.ms_sample_assay_plan.sample_plan, 0
            )
            * first_arm.group_size
        )
        expected_num_of_samples_nmr_plan_second_arm = (
            reduce(
                lambda acc_value, sample_node: acc_value + sample_node.size, self.nmr_sample_assay_plan.sample_plan, 0
            )
            * second_arm.group_size
        )
        expected_num_of_samples_tot = (
            2 * expected_num_of_samples_nmr_plan_second_arm
            + expected_num_of_samples_ms_plan_first_arm
            + expected_num_of_samples_nmr_plan_first_arm
        )
        ...
        ms_processes = [
            process
            for process in ms_assay.process_sequence
    ...
    def test_generate_isa_study_two_arms_single_cell_elements_check_source_characteristics(self):
        ...
        treatment_arm = StudyArm(
            ...
            source_type=treatment_source_type,
            group_size=20,
            arm_map=OrderedDict(
                [
                    (self.cell_screen, None),
    ...
    def test_study_design_repr(self):
        self.assertEqual(
            repr(self.study_design),
            """isatools.create.model.StudyDesign(identifier=None, name=Study Design, design_type=None, description=None source_type=Characteristic(
\tcategory=Study Subject
\tvalue=OntologyAnnotation(
\tterm=Human
\tterm_source=NCIT
...
class QualityControlServiceTest(BaseStudyDesignTest):
    ...
    def test_expansion_of_single_mass_spectrometry_assay(self):
        ...
        ms_assay_no_qc = next(
            assay for assay in study_no_qc.assays if assay.technology_type == ms_assay_dict["technology_type"]
        )
        expected_num_of_samples_ms_plan_first_arm = (
            reduce(lambda acc_value, sample_node: acc_value + sample_node.size, ms_sample_assay_plan.sample_plan, 0)
            * first_arm.group_size
        )
        ms_processes = [
            process
            for process in ms_assay_no_qc.process_sequence
            if process.executes_protocol.name.endswith("mass spectrometry")
        ]
        self.assertEqual(len(ms_processes), 2 * 2 * 2 * 2 * expected_num_of_samples_ms_plan_first_arm)
        ...
        log.debug("Sample name occurrences: {}".format(json.dumps(Counter(sample_names), sort_keys=True, indent=2)))
        self.assertEqual(len(sample_names), len(set(sample_names)))  # all sample names are unique
        ms_assay_no_qc = next(
            assay for assay in study_no_qc.assays if assay.technology_type == ms_assay_dict["technology_type"]
        )
        ms_assay_with_qc = next(
            assay for assay in study_with_qc.assays if assay.technology_type == ms_assay_dict["technology_type"]
        )
        ...
        ms_processes = [
            process
            for process in ms_assay_with_qc.process_sequence
            if process.executes_protocol.name.endswith("mass spectrometry")
        ]
        log.debug(
            "QC pre-run sample size: {0}, QC post-run sample size: {1}, QC interspersed samples: {2}".format(
                self.qc.pre_run_sample_type.size,
                self.qc.post_run_sample_type.size,
        ...
        expected_num_of_interspersed_samples = (
            expected_num_of_samples_ms_plan_first_arm - 1
        ) // self.interspersed_sample_types[0][1]
        log.debug("expected number of interspersed samples: {0}".format(expected_num_of_interspersed_samples))
        qc_samples_size = (
            self.qc.pre_run_sample_type.size + self.qc.post_run_sample_type.size + expected_num_of_interspersed_samples
        )
        log.debug("expected qc_samples_size: {0}".format(qc_samples_size))
        self.assertEqual(
            len(ms_processes), 2 * 2 * 2 * 2 * (expected_num_of_samples_ms_plan_first_arm + qc_samples_size)
        )
...
class TreatmentFactoryTest(unittest.TestCase):
    ...
    def test_compute_full_factorial_design(self):
        ...
        self.assertEqual(
            ...
            {
                Treatment(
                    element_type=INTERVENTIONS["CHEMICAL"],
                    factor_values=(
...
class StudyDesignFactoryTest(unittest.TestCase):
            ...
            StudyArm(
                ...
                arm_map=OrderedDict(
                    [
                        ...
                        (StudyCell("ARM_00_CELL_02", elements=(self.first_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_00_CELL_03", elements=(self.washout,)), None),
                        (StudyCell("ARM_00_CELL_04", elements=(self.second_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_00_CELL_05", elements=(self.washout,)), None),
                        (StudyCell("ARM_00_CELL_06", elements=(self.third_treatment,)), self.sample_assay_plan),
            ...
            StudyArm(
                ...
                arm_map=OrderedDict(
                    [
                        ...
                        (StudyCell("ARM_01_CELL_02", elements=(self.first_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_01_CELL_03", elements=(self.washout,)), None),
                        (StudyCell("ARM_01_CELL_04", elements=(self.third_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_01_CELL_05", elements=(self.washout,)), None),
                        (StudyCell("ARM_01_CELL_06", elements=(self.second_treatment,)), self.sample_assay_plan),
            ...
            StudyArm(
                ...
                arm_map=OrderedDict(
                    [
                        ...
                        (StudyCell("ARM_02_CELL_02", elements=(self.second_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_02_CELL_03", elements=(self.washout,)), None),
                        (StudyCell("ARM_02_CELL_04", elements=(self.first_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_02_CELL_05", elements=(self.washout,)), None),
                        (StudyCell("ARM_02_CELL_06", elements=(self.third_treatment,)), self.sample_assay_plan),
    ...
    def test_compute_parallel_design_three_treatments(self):
        ...
        parallel_design = StudyDesignFactory.compute_parallel_design(
            treatments_map,
            ...
            StudyArm(
                ...
                arm_map=OrderedDict(
                    [
                        (StudyCell("ARM_01_CELL_00", elements=(self.screen,)), None),
                        (StudyCell("ARM_01_CELL_01", elements=(self.run_in,)), None),
                        (StudyCell("ARM_01_CELL_02", elements=(self.second_treatment,)), self.sample_assay_plan),
            ...
            StudyArm(
                ...
                arm_map=OrderedDict(
                    [
                        (StudyCell("ARM_02_CELL_00", elements=(self.screen,)), None),
                        (StudyCell("ARM_02_CELL_01", elements=(self.run_in,)), None),
                        (StudyCell("ARM_02_CELL_02", elements=(self.third_treatment,)), self.sample_assay_plan),
    ...
    def test_compute_parallel_design_group_sizes_another_bad_treatment_error(self):
        another_bad_treatments_map = [
            (self.first_treatment, self.sample_assay_plan),
            (self.second_treatment, self.sample_assay_plan),
            ("toto"),
        ]  # last element is not a tuple, not a treatment but a string
        with self.assertRaises(TypeError) as ex_cm:
    ...
    def test_1(self):
        ...
        with self.assertRaises(TypeError) as ex_cm:
            self.factory._validate_maps_multi_element_cell(
                treatments_map, incorrect_treatments, not_a_washout, not_a_screen_map
            )
        self.assertEqual(ex_cm.exception.args[0], errors.TREATMENT_MAP_ERROR)
    ...
    def test_compute_single_arm_design_tree_treatments(self):
        ...
        parallel_design = StudyDesignFactory.compute_single_arm_design(
            treatments_map,
            group_size=19,
            screen_map=(self.screen, None),
            run_in_map=(self.run_in, None),
            ...
            StudyArm(
                ...
                arm_map=OrderedDict(
                    [
                        (StudyCell("ARM_00_CELL_00", elements=(self.screen,)), None),
                        (StudyCell("ARM_00_CELL_01", elements=(self.run_in,)), None),
                        (StudyCell("ARM_00_CELL_02", elements=(self.second_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_00_CELL_03", elements=(self.washout,)), None),
                        (StudyCell("ARM_00_CELL_04", elements=(self.fourth_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_00_CELL_05", elements=(self.washout,)), None),
                        (StudyCell("ARM_00_CELL_06", elements=(self.third_treatment,)), self.sample_assay_plan),
                        (StudyCell("ARM_00_CELL_07", elements=(self.follow_up,)), self.sample_assay_plan),
                    ]
    ...
    def test_compute_concomitant_treatment_design_three_treatments(self):
        ...
        self.assertEqual(
            list(concomitant_treatment_design.study_arms[0].arm_map.keys())[0],
            StudyCell(
                "ARM_00_CELL_00", elements=({self.fourth_treatment, self.second_treatment, self.first_treatment},)
            ),
        )
        with self.assertRaises(TypeError) as ex_cm:
            self.factory._validate_maps_multi_element_cell(
                treatments, self.sample_assay_plan, follow_up_map=("", self.sample_assay_plan)
            )
                ...
                arm_map=OrderedDict(
                    [
                        (
                            StudyCell(
                                "ARM_00_CELL_00",
                                elements=({self.fourth_treatment, self.second_treatment, self.first_treatment},),
                            ),
    ...
    def test_compute_concomitant_treatment_design_three_treatments_screen(self):
        ...
        self.assertEqual(
            list(concomitant_treatment_design.study_arms[0].arm_map.keys())[0],
            StudyCell("ARM_00_CELL_00", elements=(self.screen,)),
            self.sample_assay_plan,
        )
        """
        self.assertEqual(repr(list(concomitant_treatment_design.study_arms[0].arm_map.keys())[0].elements),
    ...
    def test_compute_concomitant_treatment_design_three_treatments_run_in(self):
        ...
        concomitant_treatment_design = StudyDesignFactory.compute_concomitant_treatments_design(
            ...
            self.sample_assay_plan,
            group_size=30,
            run_in_map=(self.run_in, self.sample_assay_plan),
            screen_map=(self.screen, self.sample_assay_plan),
        )
        self.assertEqual(len(concomitant_treatment_design.study_arms), 1)
        self.assertEqual(
            list(concomitant_treatment_design.study_arms[0].arm_map.keys())[1],
            StudyCell("ARM_00_CELL_01", elements=(self.run_in,)),
            self.sample_assay_plan,
        )
        """
        self.assertEqual(repr(list(concomitant_treatment_design.study_arms[0].arm_map.keys())[0].elements),
    ...
    def test_compute_crossover_design_multi_element_cell_three_treatments(self):
        ...
        # six permutations
        self.assertEqual(
            crossover_design_with_multi_element_cell.study_arms[0],
            StudyArm(
                name="ARM_00",
                group_size=10,
                arm_map=OrderedDict(
        ...
        self.assertEqual(
            crossover_design_with_multi_element_cell.study_arms[1],
            StudyArm(
                name="ARM_01",
                group_size=15,
                arm_map=OrderedDict(
        ...
        self.assertEqual(
            crossover_design_with_multi_element_cell.study_arms[2],
            StudyArm(
                name="ARM_02",
                group_size=12,
                arm_map=OrderedDict(
