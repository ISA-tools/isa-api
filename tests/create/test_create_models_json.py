...
class OrderedTest(unittest.TestCase):
    def test_lists_of_lists(self):
        test_list = [{"id": 3}, {"id": 1}, {"id": 0}, [{"id": 2}, {"id": 7}, {"id": 5}], {"id": 6}]
...
class BaseTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.fifth_treatment = Treatment(
            element_type=INTERVENTIONS["DIETARY"],
            factor_values=(
        ...
        self.sixth_treatment = Treatment(
            element_type=INTERVENTIONS["RADIOLOGICAL"],
            factor_values=(
                FactorValue(factor_name=BASE_FACTORS[0], value=RADIOLOGICAL_FACTOR_0_VALUE),
                FactorValue(
                    factor_name=BASE_FACTORS[1], value=RADIOLOGICAL_FACTOR_1_VALUE, unit=RADIOLOGICAL_FACTOR_1_UNIT
                ),
                FactorValue(
        ...
        self.seventh_treatment = Treatment(
            element_type=INTERVENTIONS["BIOLOGICAL"],
            factor_values=(
                FactorValue(factor_name=BASE_FACTORS[0], value=BIOLOGICAL_FACTOR_0_VALUE),
                FactorValue(
                    factor_name=BASE_FACTORS[1], value=BIOLOGICAL_FACTOR_1_VALUE, unit=BIOLOGICAL_FACTOR_1_UNIT
                ),
                FactorValue(
                    factor_name=BASE_FACTORS[2], value=BIOLOGICAL_FACTOR_2_VALUE, unit=BIOLOGICAL_FACTOR_2_UNIT
                ),
            ),
        )
        ...
        self.cell_multi_elements_bio_diet = StudyCell(
            "MULTI-ELEMENT CELL BIO-DIET",
            elements=[
                {self.second_treatment, self.fourth_treatment, self.first_treatment},
                self.washout,
                self.fifth_treatment,
                self.washout,
        ...
        self.mouse_source_type = Characteristic(
            ...
            value=OntologyAnnotation(
                term="Mouse",
                term_accession="http://purl.obolibrary.org/obo/NCIT_C14238",
                term_source=default_ontology_source_reference,
            ),
        )
...
class CharacteristicEncoderTest(unittest.TestCase):
    ...
    def test_with_ontology_annotations(self):
        ...
        characteristic = Characteristic(
            category=OntologyAnnotation(
                term="length", term_accession="http://purl.obolibrary.org/obo/NCIT_C25334", term_source=ncit
            ),
            value=200,
            unit=OntologyAnnotation(
                term="meter", term_accession="http://purl.obolibrary.org/obo/NCIT_C41139", term_source=ncit
            ),
        )
    ...
    def test_with_strings(self):
        ...
        actual_json_characteristic = json.loads(json.dumps(characteristic, cls=CharacteristicEncoder))
        expected_json_characteristic = {
            "category": {"term": characteristic.category.term},
            "value": characteristic.value,
        }
        self.assertEqual(ordered(actual_json_characteristic), ordered(expected_json_characteristic))
...
class CharacteristicDecoderTest(unittest.TestCase):
    ...
    def test_characteristics_string(self):
        characteristic_string_dict = {"category": "Time", "value": 126.4, "unit": "sec."}
...
class StudyCellEncoderTest(BaseTestCase):
    ...
    def test_encode_single_treatment_cell(self):
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "..", "data", "json", "create", "single-treatment-cell.json")
        ) as expected_json_fp:
    ...
    def test_encode_multi_treatment_cell(self):
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "..", "data", "json", "create", "multi-treatment-padded-cell.json")
        ) as expected_json_fp:
            expected_json_cell = json.load(expected_json_fp)
...
class SampleAndAssayPlanEncoderAndDecoderTest(unittest.TestCase):
    def setUp(self):
        ...
        self.third_assay_graph = AssayGraph(
            id_="assay-graph/02",
        ...
        self.tissue_node = ProductNode(
            id_="product-node/0000", name="tissue", node_type=SAMPLE, size=2, characteristics=[self.tissue_char]
        )
        self.blood_node = ProductNode(
            id_="product-node/0001", name="blood", node_type=SAMPLE, size=3, characteristics=[self.blood_char]
        )
        ...
        self.extraction_instrument = ParameterValue(
            category=ProtocolParameter(parameter_name="instrument"), value="Maxwell RSC 48"
        )
        self.protocol_node_dna = ProtocolNode(
            id_="protocol-node/0000",
            name="DNA extraction",
            version="1.0.0",
            parameter_values=[self.extraction_instrument],
        )
        self.protocol_node_rna = ProtocolNode(
            id_="protocol-node/0001",
            name="RNA extraction",
            version="0.1",
            parameter_values=[self.extraction_instrument],
        )
        self.dna_node = ProductNode(
            id_="product-node/0002", name="DNA", node_type=EXTRACT, size=3, characteristics=[self.dna_char]
        )
        self.mrna_node = ProductNode(
            id_="product-node/0003", name="mRNA", node_type=EXTRACT, size=3, characteristics=[self.mrna_char]
        )
        self.mirna_node = ProductNode(
            id_="product-node/0004", name="miRNA", node_type=EXTRACT, size=5, characteristics=[self.mirna_char]
        )
        ...
        self.second_assay_graph.add_links(
            [(self.protocol_node_rna, self.mirna_node), (self.protocol_node_rna, self.mrna_node)]
        )
    ...
    def test_encode_dna_rna_extraction_plan(self):
        ...
        with open(
            os.path.join(
                ...
                "json",
                "create",
                "dna-rna-extraction-sample-and-assay-plan.json",
            )
        ) as expected_json_fp:
    ...
    def test_encode_sample_and_assay_plan_with_ontology_annotations(self):
        ...
        assay_node_json = next(
            node for node in actual_json_plan["assayPlan"][0]["nodes"] if node["@id"] == "nmr_spectroscopy_000_000"
        )
...
class StudyArmEncoderTest(BaseTestCase):
    ...
    def test_encode_arm_with_single_element_cells(self):
        ...
        with open(
            os.path.join(
                os.path.dirname(__file__), "..", "data", "json", "create", "study-arm-with-single-element-cells.json"
            )
        ) as expected_json_fp:
    ...
    def test_encode_arm_with_multi_element_cell(self):
        ...
        with open(
            os.path.join(
                os.path.dirname(__file__), "..", "data", "json", "create", "study-arm-with-multi-element-cell.json"
            )
        ) as expected_json_fp:
            expected_json_arm = json.load(expected_json_fp)
        self.assertEqual(ordered(actual_json_arm), ordered(expected_json_arm))
    ...
    def test_decode_arm_with_multi_element_cells_mouse(self):
        ...
        with open(
            os.path.join(
                ...
                "json",
                "create",
                "study-arm-with-multi-element-cell-mouse.json",
            )
        ) as expected_json_fp:
            json_text = json.dumps(json.load(expected_json_fp))
            actual_arm = decoder.loads(json_text)
...
class StudyDesignEncoderTest(BaseTestCase):
    def setUp(self):
        ...
        self.multi_element_cell_two_arm_study_design = StudyDesign(
            ...
            design_type="unspecified design",
            study_arms=[self.multi_treatment_cell_arm, self.multi_treatment_cell_arm_01],
        )
    ...
    def test_encode_study_design_with_three_arms(self):
        ...
        with open(
            os.path.join(
                ...
                "json",
                "create",
                "study-design-with-three-arms-single-element-cells.json",
            )
        ) as expected_json_fp:
    ...
    def test_encode_study_design_with_two_arms_with_multi_element_cells(self):
        actual_json_study_design = json.loads(
            json.dumps(self.multi_element_cell_two_arm_study_design, cls=StudyDesignEncoder)
        )
        with open(
            os.path.join(
                os.path.dirname(__file__),
                ...
                "json",
                "create",
                "study-design-with-two-arms-multi-element-cells.json",
            )
        ) as expected_json_fp:
            expected_json_study_design = json.load(expected_json_fp)
        self.assertEqual(ordered(actual_json_study_design), ordered(expected_json_study_design))
