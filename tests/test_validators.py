...
class TestValidateIsaJson(unittest.TestCase):
    ...
    def test_validate_isajson_json_load(self):
        """Tests against 0002"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'minimal_syntax.json')))
        if 2 in [e['code'] for e in report['errors']]:
            self.fail("Error raised when trying to parse JSON, when it should have been fine!")
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'invalid.json')))
    ...
    def test_validate_isajson_isajson_schemas(self):
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'invalid_isajson.json')))
    ...
    def test_validate_isajson_utf8_encoding_check(self):
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'non_utf8.json')))
    ...
    def test_validate_isajson_source_link(self):
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'source_link.json')))
        if 1005 in [e['code'] for e in report['errors']]:
            self.fail("Validation error present when should pass without error - source link reports broken when "
                      "present in data")
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'source_link_fail.json')))
    ...
    def test_validate_isajson_sample_link(self):
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'sample_link.json')))
        if 1005 in [e['code'] for e in report['errors']]:
            self.fail(
                "Validation error present when should pass without error - sample link reports broken when present in "
                "data")
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'sample_link_fail.json')))
        if 1005 not in [e['code'] for e in report['errors']]:
            self.fail(
    ...
    def test_validate_isajson_data_file_link(self):
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'datafile_link.json')))
        if 1005 in [e['code'] for e in report['errors']]:
            self.fail(
                "Validation error present when should pass without error - data file link reports broken when present "
                "in data")
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'datafile_link_fail.json')))
        if 1005 not in [e['code'] for e in report['errors']]:
            self.fail(
    ...
    def test_validate_isajson_material_link(self):
        """Tests against 1005"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'material_link.json')))
        if 1005 in [e['code'] for e in report['errors']]:
            self.fail(
                "Validation error present when should pass without error -material link link reports broken when "
                "present in data")
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'material_link_fail.json')))
        if 1005 not in [e['code'] for e in report['errors']]:
    ...
    def test_validate_isajson_process_link(self):
        """Tests against 1006"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'process_link.json')))
        if 1006 in [e['code'] for e in report['errors']]:
            self.fail(
                ...
                "in data")
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'process_link_fail.json')))
    ...
    def test_validate_isajson_protocol_ref_link(self):
        """Tests against 1007"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'protocol_ref_link.json')))
        if 1007 in [e['code'] for e in report['errors']]:
            self.fail(
                ...
                "present in data")
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'protocol_ref_link_fail.json')))
    ...
    def test_validate_isajson_factor_link(self):
        """Tests against 1008"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'factor_link.json')))
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'factor_link_fail.json')))
    ...
    def test_validate_isajson_protocol_parameter_link(self):
        """Tests against 1009"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'protocol_parameter_link.json')))
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'protocol_parameter_link_fail.json')))
    ...
    def test_validate_isajson_iso8601(self):
        """Tests against 3001"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'iso8601.json')))
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'iso8601_fail.json')))
    ...
    def test_validate_isajson_doi(self):
        """Tests against 3002"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'doi.json')))
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'doi_fail.json')))
    ...
    def test_validate_isajson_pubmed(self):
        """Tests against 3003"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'pubmed.json')))
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'pubmed_fail.json')))
    ...
    def test_validate_isajson_protocol_used(self):
        """Tests against 1019"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'protocol_used.json')))
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'protocol_used_fail.json')))
    ...
    def test_validate_isajson_factor_used(self):
        """Tests against 1021"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'factor_used.json')))
        ...
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'factor_used_fail.json')))
    ...
    def test_validate_isajson_study_config_validation(self):
        """Tests against 4004"""
        report = isajson.validate(open(os.path.join(self._unit_json_data_dir, 'study_config.json')))
