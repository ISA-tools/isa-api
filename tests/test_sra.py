...
def setUpModule():
    if not os.path.exists(utils.DATA_DIR):
        raise FileNotFoundError(
            ...
            "the data repository using "
            "git clone -b tests --single-branch "
            "git@github.com:ISA-tools/data {0}".format(utils.DATA_DIR)
        )
...
class TestSraExport(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(self._json_data_dir, study_id, study_id + ".json")) as json_fp:
            self._inv_obj = isajson.load(json_fp)

        with open(os.path.join(self._json_data_dir, study_id_paired, study_id + ".json")) as json_fp:
            self._inv_obj = isajson.load(json_fp)
        ...
        self._paired_study_sra_data_dir = os.path.join(self._sra_data_dir, study_id_paired)
        with open(os.path.join(self._paired_study_sra_data_dir, "submission.xml"), "rb") as sub_fp:
            self._expected_submission_xml_obj_paired = etree.fromstring(sub_fp.read())
        with open(os.path.join(self._paired_study_sra_data_dir, "project_set.xml"), "rb") as ps_fp:
            self._expected_project_set_xml_obj_paired = etree.fromstring(ps_fp.read())
        with open(os.path.join(self._paired_study_sra_data_dir, "sample_set.xml"), "rb") as ss_fp:
            self._expected_sample_set_xml_obj_paired = etree.fromstring(ss_fp.read())
        with open(os.path.join(self._paired_study_sra_data_dir, "experiment_set.xml"), "rb") as es_fp:
            self._expected_exp_set_xml_obj_paired = etree.fromstring(es_fp.read())
        with open(os.path.join(self._paired_study_sra_data_dir, "run_set.xml"), "rb") as rs_fp:
            self._expected_run_set_xml_obj_paired = etree.fromstring(rs_fp.read())
    ...
    def test_create_datafile_hashes_success(self):
        ...
        self.assertEqual(datafilehashes["1EU.sff"], "d41d8cd98f00b204e9800998ecf8427e")  # empty file hash
