...
def setUpModule():
    if not os.path.exists(utils.DATA_DIR):
        raise FileNotFoundError(
            "Could not fine test data directory in {0}. Ensure you have cloned the "
            "data repository using "
...
class TestIsaTab2Json(unittest.TestCase):
    ...
    def test_isatab2json_convert_bii_i_1(self):
        ...
        actual_json = isatab2json.convert(
            os.path.join(self._tab_data_dir, test_case), validate_first=False, use_new_parser=True
        )
    ...
    def test_isatab2json_convert_comment(self):
        ...
        self.assertEqual(
            actual_json["studies"][0]["assays"][0]["comments"][0]["value"],
