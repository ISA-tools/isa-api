...
def setUpModule():
    if not os.path.exists(utils.DATA_DIR):
        raise FileNotFoundError(
            "Could not fine test data directory in {0}. Ensure you have cloned the data repository using "
    ...
    def test_validate_isajson_source_link(self):
        ...
        with open(os.path.join(self._unit_json_data_dir, "source_link.json")) as fp:
            ...
            if 1005 in [e["code"] for e in report["errors"]]:
                self.fail(
                    "Validation error present when should pass without error - source link reports broken when "
                    "present in data"
                )
        with open(os.path.join(self._unit_json_data_dir, "source_link_fail.json")) as fp:
            ...
            if 1005 not in [e["code"] for e in report["errors"]]:
                self.fail(
                    "Validation error missing when should report error - data has broken source link but not "
    ...
    def test_validate_isajson_protocol_parameter_link(self):
        ...
        with open(os.path.join(self._unit_json_data_dir, "protocol_parameter_link_fail.json")) as fp:
            ...
            if 1009 not in [e["code"] for e in report["errors"]]:
                self.fail(
                    "Validation error missing when should report error - data has broken parameter link in "
...
class TestStudyGroupsValidationIsaTab(unittest.TestCase):
    ...
    def test_info_reporting_bii_i_1_with_study_groups_comment_isatab(self):
        test_case = "BII-I-1"
        with open(
            os.path.join(utils.TAB_DATA_DIR, test_case, "_i_investigation_with_study_groups_comment.txt")
        ) as test_case_fp:
            report = isatab.validate(
                fp=test_case_fp, config_dir=utils.DEFAULT2015_XML_CONFIGS_DATA_DIR, log_level=self._reporting_level
            )
            self.assertIn(
                {
                    ...
                    "supplemental": "Study group size reported as 7 but found 18 in s_BII-S-1.txt",
                    "code": 5002,
                },
                report["warnings"],
            )
