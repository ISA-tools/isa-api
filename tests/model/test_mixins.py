...
class TestMetadataMixin(TestCase):
    ...
    def test_init(self):
        ...
        metadata_mixin = MetadataMixin(publications=[publication], contacts=[contact])
...
class TestStudyAssayMixin(TestCase):
    ...
    def test_units(self):
        ...
        self.assertEqual(
            str(context.exception), "StudyAssayMixin.units must be iterable containing OntologyAnnotations"
        )
    ...
    def test_other_material(self):
        ...
        self.assertEqual(str(context.exception), "StudyAssayMixin.other_material must be iterable containing Materials")
    ...
    def test_materials(self, mock_warn):
        ...
        mock_warn.assert_called_with(
            "the `materials` dict property is being deprecated in favour of `sources`, "
            "`samples`, and `other_material` properties.",
            DeprecationWarning,
        )
    ...
    def test_process_sequence(self):
        ...
        self.assertEqual(
            str(context.exception), "StudyAssayMixin.process_sequence must be iterable containing Processes"
        )
    ...
    def test_shuffle_error(self):
        with self.assertRaises(ValueError) as context:
            ...
            self.assertTrue(
                "foo should be in samples, sources, Extract Name, Labeled Extract Name" in context.exception
            )
