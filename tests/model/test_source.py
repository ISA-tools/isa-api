...
class TestSource(TestCase):
    ...
    def test_to_dict(self):
        ...
        expected_dict["characteristics"] = [{"category": {"@id": "test_id"}, "comments": [], "value": None}]
        self.assertEqual(self.source.to_dict(), expected_dict)

    def test_from_dict(self):
        expected_dict = {"@id": "source_id", "name": "source name", "comments": [], "characteristics": []}
        source = Source()
        source.from_dict(expected_dict)
        self.assertEqual(source.to_dict(), expected_dict)

        expected_dict["characteristics"] = [
            {"category": {"@id": "category_id"}, "comments": [], "value": "123", "unit": ""}
        ]
        characteristics_index = {"category_id": OntologyAnnotation(term="my category", id_="my_cat_id")}
        indexes.characteristic_categories = characteristics_index
        source.from_dict(expected_dict)
        expected_characteristics = [{"category": {"@id": "my_cat_id"}, "value": "123", "comments": []}]
