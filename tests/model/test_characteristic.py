...
class TestCharacteristic(TestCase):
    ...
    def test_category(self):
        ...
        expected_error = (
            "Characteristic.category must be either a string ot an OntologyAnnotation, or None; got 1:<class 'int'>"
        )
    ...
    def test_value(self):
        ...
        expected_error = (
            "Characteristic.value must be a string, numeric, an OntologyAnnotation, or None; got ():<class 'tuple'>"
        )
        with self.assertRaises(AttributeError) as context:
    ...
    def test_unit(self):
        ...
        expected_error = (
            "Characteristic.unit must be either a string ot an OntologyAnnotation, or None; got 1:<class 'int'>"
        )
        with self.assertRaises(AttributeError) as context:
    ...
    def test_from_dict(self):
        input_dict = {"category": "", "comments": [], "value": {}}
        characteristic = Characteristic()
        expected_error = (
            "Can't create value as annotation: 'annotationValue' object: {'category': '', 'comments': [], 'value': {}}"
        )
        ...
        input_dict = {
            "category": category,
            "comments": [],
            "value": {"@id": "test_id", "annotationValue": 123, "termSource": "", "termAccession": "", "comments": []},
        }
        characteristic.from_dict(input_dict)
