...
class ISATabTest(TestCase):
    ...
    def test_isatab_to_sra_bii_s_3(self):
        ...
        with ZipFile(res) as zip_file:  # isatab2sra returns a zip
            self.assertGreater(len(zip_file.namelist()), 0)
        self.assertTrue(os.path.exists(os.path.join(self._tmp, 'sra', 'BII-S-3')))  # but also writes to /sra/{study_id}
        ...
        if os.path.exists(expected_sra_path):
            ...
            self.assertEqual(etree.tostring(test_experiment_set_xml_obj), etree.tostring(expected_experiment_set_xml_obj))
