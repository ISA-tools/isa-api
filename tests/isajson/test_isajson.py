...
def create_descriptor(use_strings_for_characteristic_categories=False):
    ...
    characteristic_organ = Characteristic(
        ...
        if not use_strings_for_characteristic_categories
        else "OrganismPart",
        value=OntologyAnnotation(
...
class TestIsaJson(unittest.TestCase):
    ...
    def test_json_load_and_dump_bii_i_1(self):
        ...
        with open(os.path.join(utils.JSON_DATA_DIR, "BII-I-1", "BII-I-1.json")) as isajson_fp:
            ...
            self.assertListEqual(
                [s["filename"] for s in ISA_J["studies"]], ["s_BII-S-1.txt", "s_BII-S-2.txt"]
            ...
            self.assertEqual(
                len(assay_proteome["materials"]["otherMaterials"]), 19
            )  # 19 other materials in a_proteome.txt
            ...
            )  # 92 other materials in a_metabolome.txt
            ...
            self.assertEqual(
                len(assay_transcriptome["materials"]["samples"]), 48
            )  # 48 assay samples in a_transcriptome.txt
            self.assertEqual(
                len(assay_transcriptome["materials"]["otherMaterials"]), 96
            )  # 96 other materials in a_transcriptome.txt
            self.assertEqual(len(assay_transcriptome["dataFiles"]), 49)  # 49 data files  in a_transcriptome.txt
            self.assertEqual(
                len(assay_transcriptome["processSequence"]), 193
            )  # 203 processes in in a_transcriptome.txt
            ...
            )  # 28 other materials in a_microarray.txt
    ...
    def test_json_load_and_dump_bii_s_3(self):
        ...
        with open(os.path.join(utils.JSON_DATA_DIR, "BII-S-3", "BII-S-3.json")) as isajson_fp:
            ...
            self.assertListEqual(
                [s["filename"] for s in ISA_J["studies"]], ["s_BII-S-3.txt"]
            )  # 1 studies in i_gilbert.txt
            ...
            self.assertEqual(
                len(assay_gx["materials"]["otherMaterials"]), 4
            )  # 4 other materials in a_gilbert-assay-Gx.txt
            ...
            self.assertEqual(
                len(assay_tx["materials"]["otherMaterials"]), 4
            )  # 4 other materials in a_gilbert-assay-Tx.txt
    ...
    def test_json_load_and_dump_bii_s_7(self):
        ...
        with open(os.path.join(utils.JSON_DATA_DIR, "BII-S-7", "BII-S-7.json")) as isajson_fp:
            ...
            self.assertListEqual(
                [s["filename"] for s in ISA_J["studies"]], ["s_BII-S-7.txt"]
            )  # 1 studies in i_gilbert.txt
            ...
            self.assertEqual(
                len(assay_gx["materials"]["otherMaterials"]), 29
            )  # 29 other materials in a_matteo-assay-Gx.txt
    ...
    def test_isajson_char_quant_unit(self):
        ...
        source.characteristics.append(
            Characteristic(
                ...
                unit=OntologyAnnotation(
                    term="kilogram", term_source=onto_src, term_accession="http://purl.obolibrary.org/obo/UO_0000009"
                ),
            )
        )
        ...
        sample_collection_process.parameter_values = [
            ParameterValue(
                ...
                value=OntologyAnnotation(term="eppendorf tube", term_source=onto_src, term_accession="purl.org"),
            ),
            ParameterValue(
        ...
        isa_j = json.loads(
            json.dumps(investigation, cls=isajson.ISAJSONEncoder, sort_keys=True, indent=4, separators=(",", ": "))
        )
